...
*/
static int __lfind(LF_SLIST * volatile *head, CHARSET_INFO *cs, uint32 hashnr,
                         ...
                         LF_SLIST *node, LF_PINS *pins, uint flags)
{
  ...
  for (;;)
  {
    if (__lfind(head, cs, node->hashnr, node->key, node->keylen,
              &cursor, pins) &&
                   ...
                   const uchar *key, uint keylen, LF_PINS *pins)
{
  ...
  for (;;)
  {
    if (!__lfind(head, cs, hashnr, key, keylen, &cursor, pins))
    {
      res= 1; /* not found */
      break;
    }
    else
    {
                           ...
                           (void *)(((intptr)cursor.next) | 1)))
      {
        ...
        else
        {
          ...
          */
          __lfind(head, cs, hashnr, key, keylen, &cursor, pins);
        }
        res= 0;
        break;
      }
...
*/
static LF_SLIST *__lsearch(LF_SLIST * volatile *head, CHARSET_INFO *cs,
                         uint32 hashnr, const uchar *key, uint keylen,
                         LF_PINS *pins)
{
  CURSOR cursor;
  int res= __lfind(head, cs, hashnr, key, keylen, &cursor, pins);
  if (res)
    _lf_pin(pins, 2, cursor.curr);
  _lf_unpin(pins, 0);
  _lf_unpin(pins, 1);
...
/*
  ...
  NOTE
    see __lsearch() for pin usage notes
*/
void *lf_hash_search(LF_HASH *hash, LF_PINS *pins, const void *key, uint keylen)
{
  ...
  if (*el == NULL && unlikely(initialize_bucket(hash, el, bucket, pins)))
    return MY_ERRPTR;
  found= __lsearch(el, hash->charset, my_reverse_bits(hashnr) | 1,
                 (uchar *)key, keylen, pins);
  lf_rwunlock_by_pins(pins);
