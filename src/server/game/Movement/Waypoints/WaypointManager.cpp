...
void WaypointMgr::Load()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 waypoints. DB table `waypoint_data` is empty!");
    ...
    TC_LOG_INFO("server", ">> Loaded %u waypoints in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
