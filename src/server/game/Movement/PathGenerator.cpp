...
////////////////// PathGenerator //////////////////
PathGenerator::PathGenerator(const Unit* owner) : _polyLength(0), _type(PATHFIND_BLANK), _useStraightPath(false), _forceDestination(false), _pointPathLimit(MAX_POINT_PATH_LENGTH), _straightLine(false),
    _endPosition(G3D::Vector3::zero()), _sourceUnit(owner), _navMesh(NULL), _navMeshQuery(NULL), _transport(NULL), _go(nullptr), _enableShort(true), _charges(false)
{
    memset(_pathPolyRefs, 0, sizeof(_pathPolyRefs));

    if (_sourceUnit->IsPlayer() || _sourceUnit->IsPet())
        TC_LOG_DEBUG("maps", "++ PathGenerator::PathGenerator for %u", _sourceUnit->GetGUIDLow());
...
void PathGenerator::InitPath()
{
    ...
    {
        if (Unit* _owner = _sourceUnit->GetAnyOwner())
            _transport = _owner->GetTransport();
        else
            _transport = _sourceUnit->GetTransport();

        auto mmap = MMAP::MMapFactory::createOrGetMMapManager();
        if (_transport)
            _navMeshQuery = mmap->GetModelNavMeshQuery(_transport->GetDisplayId(), _sourceUnit->GetInstanceId());
        else
        {
            DynamicTreeCallback dCallback;
            float heightZ = _sourceUnit->GetMap()->GetHeight(_sourceUnit->GetPhaseMask(), _sourceUnit->GetPositionX(), _sourceUnit->GetPositionY(), _sourceUnit->GetPositionZ(), true, DEFAULT_HEIGHT_SEARCH, &dCallback);
            if (dCallback.go && heightZ <= _sourceUnit->GetPositionZ() && (_sourceUnit->GetPositionZ() - heightZ) <= 2.3f) // Only if go under unit
                _go = dCallback.go;

            if (_go)
                _navMeshQuery = mmap->GetModelNavMeshQuery(_go->GetDisplayId(), _sourceUnit->GetInstanceId());
        }

        if (!_navMeshQuery) // If transport or go don`t have mesh disable it
        {
            _go = nullptr;
            _transport = nullptr;
        ...
        if (_sourceUnit->IsPlayer() || _sourceUnit->IsPet())
            TC_LOG_DEBUG("maps", "PathGenerator displayId %i _navMeshQuery %i _navMesh %i", _go ? _go->GetDisplayId() : 0, bool(_navMeshQuery), bool(_navMesh));
    }
}
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine)
{
    ...
    if (_transport)
        _transport->CalculatePassengerOffset(destX, destY, destZ, unused);
    else if (_go)
    {
        _go->CalculatePassengerOffset(x, y, z, unused);
        _go->CalculatePassengerOffset(destX, destY, destZ, unused);
    }
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->IsPet())
        TC_LOG_DEBUG("maps", "++ PathGenerator::CalculatePath() for %u _navMesh %u _navMeshQuery %u start %u dest %u",
            _sourceUnit->GetGUIDLow(), bool(_navMesh), bool(_navMeshQuery), HaveTile(start), HaveTile(dest));
...
bool PathGenerator::CalculateShortcutPath(float destX, float destY, float destZ)
{
    ...
    if (_sourceUnit->IsPlayer() || _sourceUnit->IsPet())
        TC_LOG_DEBUG("maps", "++ PathGenerator::CalculateShortcutPath() for %u _navMesh %u _navMeshQuery %u start %u dest %u",
            _sourceUnit->GetGUIDLow(), bool(_navMesh), bool(_navMeshQuery), HaveTile(start), HaveTile(dest));
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    if (startPoly == INVALID_POLYREF || endPoly == INVALID_POLYREF)
    {
        ...
        if (waterPath)
        {
            ...
            {
                ZLiquidStatus status = _sourceUnit->GetMap()->GetLiquidStatus(_pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z, MAP_ALL_LIQUIDS, NULL);
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    if (_straightLine)
    {
        ...
        dtResult = _navMeshQuery->findStraightPath(startPoint, endPoint, _pathPolyRefs, _polyLength, pathPoints, NULL, NULL, reinterpret_cast<int*>(&pointCount), _pointPathLimit);
    else
...
void PathGenerator::NormalizePath()
{
    ...
    if (_go)
        for (uint32 i = 0; i < _pathPoints.size(); ++i)
            _go->CalculatePassengerPosition(_pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z, unused);
    ...
    for (uint32 i = 0; next < _pathPoints.size(); ++i, ++next)
    {
        ...
        bool result = map->isInLineOfSight(_pathPoints[next].x, _pathPoints[next].y, _pathPoints[next].z + 0.5f, _pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z + 0.5f, _sourceUnit->GetPhaseMask());
        ...
        while (!result)
        {
            ...
            result = map->isInLineOfSight(lastPoint[0], lastPoint[1], lastPoint[2] + 0.5f, moveTgt[0], moveTgt[1], moveTgt[2] + 0.5f, _sourceUnit->GetPhaseMask());
            if (result)
            {
                ...
                if (!result)
                    result = map->isInLineOfSight(lastPoint[0], lastPoint[1], lastPoint[2] + 0.5f, moveCur[0], moveCur[1], moveCur[2] + 0.5f, _sourceUnit->GetPhaseMask());
            }
            else
...
{
    _filter.setExcludeFlags(NAV_STEEP_SLOPES);
    _enableShort = enable;
}
