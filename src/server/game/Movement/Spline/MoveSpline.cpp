...
void MoveSpline::init_spline(const MoveSplineInitArgs& args)
{
    ...
    {
        TC_LOG_ERROR("server", "MoveSpline::init_spline: zero length spline, wrong input data?");
...
#define CHECK(exp) \
    if (!(exp))\
    {\
        TC_LOG_ERROR("server", "MoveSplineInitArgs::Validate: expression '%s' failed", #exp);\
        return false;\
    }
#define CHECK_V(exp) \
    if (!(exp))\
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Catmullrom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size()-1; ++i)
        {
            ...
            {
                TC_LOG_ERROR("server", "MoveSplineInitArgs::_checkPathBounds check failed");
                return false;
            }
        }
    }
