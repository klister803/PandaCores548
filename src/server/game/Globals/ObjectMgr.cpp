...
void ObjectMgr::LoadMapDifficultyAddon()
{
    QueryResult result = WorldDatabase.Query("SELECT mapId, difficulty, resetTime, maxPlayers, hasErrorMessage FROM map_difficulty_addon");
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 map difficulties. DB table `map_difficulty_addon` is empty.");
        return;
    }
    ...
    do
    {
        ...
        sMapDifficultyMap[MAKE_PAIR32(mapId, difficulty)] = MapDifficulty(resetTime, maxPlayers, hasErrorMessage);
        ++count;
    }
    while (result->NextRow());

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u map difficulties.", count);
}
