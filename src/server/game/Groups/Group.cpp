...
Group::~Group()
{
    if (m_bgGroup)
    {
        TC_LOG_DEBUG("bg", "Group::~Group: battleground group being deleted.");
        ...
        else TC_LOG_ERROR("server", "Group::~Group: battleground group is not linked to the correct battleground.");
    }
    Rolls::iterator itr;
    while (!RollId.empty())
    {
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    for (i = loot->items.begin(); i != loot->items.end(); ++i, ++itemSlot)
    {
        ...
        if (!item)
        {
            //TC_LOG_DEBUG("server", "Group::GroupLoot: missing item prototype for item with id: %d", i->itemid);
            continue;
        }
...
void Group::UpdatePlayerOutOfRange(Player* player)
{
    ...
    for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        if (member && !member->IsWithinDist(player, member->CalcVisibilityRange(), false))
...
void Group::BroadcastGroupUpdate(void)
{
    ...
    for (member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        if (pp && pp->IsInWorld())
        {
            ...
            TC_LOG_DEBUG("server", "-- Forced group value update for '%s'", pp->GetName());
        }
    }
}
