...
uint32 GroupMgr::GenerateNewGroupDbStoreId()
{
    ...
    {
        TC_LOG_ERROR("server", "Group storage ID overflow!! Can't continue, shutting down server. ");
...
uint32 GroupMgr::GenerateGroupId()
{
    ...
    {
        TC_LOG_ERROR("server", "Group guid overflow!! Can't continue, shutting down server. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
    return NextGroupId++;
}
...
{
    {
        ...
        if (!result)
        {
            TC_LOG_INFO("server", ">> Loaded 0 group definitions. DB table `groups` is empty!");
            return;
        }
        ...
        TC_LOG_INFO("server", ">> Loaded %u group definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    TC_LOG_INFO("server", "Loading Group members...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("server", ">> Loaded 0 group members. DB table `group_member` is empty!");
            return;
        }
        ...
        do
        {
            ...
            if (group)
                ...
                TC_LOG_ERROR("server", "GroupMgr::LoadGroups: Consistency failed, can't find group (storage id: %u)", fields[0].GetUInt32());

            ++count;
        }
        while (result->NextRow());

        TC_LOG_INFO("server", ">> Loaded %u group members in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    TC_LOG_INFO("server", "Loading Group instance saves...");
    {
        uint32 oldMSTime = getMSTime();
        //                                                   0           1        2              3             4             5
        ...
        if (!result)
        {
            TC_LOG_INFO("server", ">> Loaded 0 group-instance saves. DB table `group_instance` is empty!");
            return;
        }
        ...
        do
        {
            ...
            {
                TC_LOG_ERROR("sql", "Incorrect entry in group_instance table : no dungeon map %d", fields[1].GetUInt16());
                continue;
            }
            ...
            {
                TC_LOG_ERROR("sql", "Wrong dungeon difficulty use in group_instance table: %d", diff + 1);
        ...
        TC_LOG_INFO("server", ">> Loaded %u group-instance saves in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
}
