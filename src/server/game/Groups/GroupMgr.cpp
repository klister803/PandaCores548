...
void GroupMgr::LoadGroups()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Group instance saves...");
    {
        uint32 oldMSTime = getMSTime();
        //                                                   0           1        2              3             4             5            6
        QueryResult result = CharacterDatabase.Query("SELECT gi.guid, i.map, gi.instance, gi.permanent, i.difficulty, COUNT(g.guid), i.dungeonId "
        ...
        do
        {
            ...
            InstanceSave* save = sInstanceSaveMgr->AddInstanceSave(mapEntry->MapID, fields[2].GetUInt32(), Difficulty(diff), (bool)fields[5].GetUInt64(), true, fields[6].GetUInt32());
