...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 auction items. DB table `auctionhouse` or `item_instance` is empty!");
    ...
    do
    {
        ...
        if (!proto)
        {
            TC_LOG_ERROR("server", "AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: %u id: #%u) in auction, skipped.", item_guid, itemEntry);
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u auction items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server", ">> Loaded 0 auctions. DB table `auctionhouse` is empty.");
    ...
    TC_LOG_INFO("server", ">> Loaded %u auctions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data) const
{
    ...
    if (!item)
    {
        TC_LOG_ERROR("server", "AuctionEntry::BuildAuctionInfo: Auction %u has a non-existent item: %u", Id, itemGUIDLow);
        return false;
    }
    data << uint32(Id);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        TC_LOG_ERROR("server", "Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        TC_LOG_ERROR("server", "Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    {
        TC_LOG_ERROR("server", "Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
    ...
    {
        TC_LOG_ERROR("server", "Auction %u has not a existing item : %u", Id, itemGUIDLow);
        return false;
    }
    return true;
}
...
void AuctionHouseMgr::DeleteExpiredAuctionsAtStartup()
{
    ...
    if (!expAuctions)
    {
        TC_LOG_INFO("server", ">> No expired auctions to delete");
    ...
    TC_LOG_INFO("server", ">> Deleted %u expired auctions in %u ms", expirecount, GetMSTimeDiffToNow(oldMSTime));
...
bool AuctionEntry::LoadFromFieldList(Field* fields)
{
    ...
    if (!auctioneerData)
    {
        TC_LOG_ERROR("server", "AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u)", Id, auctioneer);
        return false;
    }
    ...
    if (!auctioneerInfo)
    {
        TC_LOG_ERROR("server", "AuctionEntry::LoadFromFieldList() - Auction %u has not a existing auctioneer (GUID : %u Entry: %u)", Id, auctioneer, auctioneerData->id);
        return false;
    }
    ...
    if (!auctionHouseEntry)
    {
        TC_LOG_ERROR("server", "AuctionEntry::LoadFromFieldList() - Auction %u has auctioneer (GUID : %u Entry: %u) with wrong faction %u", Id, auctioneer, auctioneerData->id, factionTemplateId);
        return false;
    }
