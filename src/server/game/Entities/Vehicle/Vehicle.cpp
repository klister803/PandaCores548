...
void Vehicle::Uninstall(bool uninstallBeforeDelete)
{
    ...
    {
        TC_LOG_ERROR("vehicles", "Vehicle GuidLow: %u, Entry: %u attempts to uninstall, but already has STATUS_UNINSTALLING! "
    ...
    TC_LOG_DEBUG("vehicles", "Vehicle::Uninstall Entry: %u, GuidLow: %u", _creatureEntry, _me->GetGUIDLow());
    RemoveAllPassengers();
...
void Vehicle::Reset(bool evading /*= false*/)
{
    ...
    TC_LOG_DEBUG("vehicles", "Vehicle::Reset (Entry: %u, GuidLow: %u, DBGuid: %u)", GetCreatureEntry(), _me->GetGUIDLow(), _me->ToCreature()->GetDBTableGUIDLow());
...
{
    TC_LOG_DEBUG("vehicles", "Vehicle::RemoveAllPassengers. Entry: %u, GuidLow: %u", _creatureEntry, _me->GetGUIDLow());
...
void Vehicle::InstallAccessory(uint32 entry, int8 seatId, bool minion, uint8 type, uint32 summonTime)
{
    ...
    {
        TC_LOG_ERROR("vehicles", "Vehicle (GuidLow: %u, DB GUID: %u, Entry: %u) attempts to install accessory (Entry: %u) on seat %d with STATUS_UNINSTALLING! "
    ...
    TC_LOG_DEBUG("vehicles", "Vehicle (GuidLow: %u, DB Guid: %u, Entry %u): installing accessory (Entry: %u) on seat: %d",
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    if (_status == STATUS_UNINSTALLING)
    {
        TC_LOG_ERROR("vehicles", "Passenger GuidLow: %u, Entry: %u, attempting to board vehicle GuidLow: %u, Entry: %u during uninstall! SeatId: %d",
    ...
    TC_LOG_DEBUG("vehicles", "Unit %s scheduling enter vehicle (entry: %u, vehicleId: %u, guid: %u (dbguid: %u) on seat %d",
        unit->GetName(), _me->GetEntry(), _vehicleInfo->m_ID, _me->GetGUIDLow(), 
...
void Vehicle::RemovePassenger(Unit* unit)
{
    ...
    TC_LOG_DEBUG("vehicles", "Unit %s exit vehicle entry %u id %u dbguid %u seat %d", 
    ...
    for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
    {
        if (Unit* passenger = ObjectAccessor::GetUnit(*GetBase(), itr->second.Passenger))
        {
            ...
            //if (passenger->GetTypeId() == TYPEID_PLAYER)
                //TC_LOG_DEBUG("spell", "RelocatePassengers loc(%f %f %f)", px, py, pz);
        }
    }
}
...
void VehicleJoinEvent::Abort(uint64)
{
    ...
    if (targetBase)
    {
        //TC_LOG_DEBUG("vehicles", "Passenger GuidLow: %u, Entry: %u, board on vehicle GuidLow: %u, Entry: %u SeatId: %d cancelled",
        ...
        TC_LOG_DEBUG("vehicles", "Passenger GuidLow: %u, Entry: %u, board on uninstalled vehicle SeatId: %d cancelled",
        Passenger->GetGUIDLow(), Passenger->GetEntry(), (int32) Seat->first);
    else
        TC_LOG_ERROR("vehicles", " WARNING!!! VehicleJoinEvent Abort with non existen Passanger");
