...
inline uint64 MAKE_NEW_GUID(uint64 l, uint64 e, uint64 h)
{
    ...
    return uint64(uint64(l) | (uint64(e) << (h == HIGHGUID_PET ? 24 : 32)) | (uint64(h) << ((h == HIGHGUID_CORPSE || h == HIGHGUID_INSTANCE_SAVE || h == HIGHGUID_AREATRIGGER) ? 48 : 52)));
}
...
inline uint32 GUID_HIPART(uint64 guid)
{
    ...
    return (t == HIGHGUID_CORPSE || t == HIGHGUID_INSTANCE_SAVE || t == HIGHGUID_AREATRIGGER) ? t : ((uint32(t) >> 4) & 0x00000FFF);
}
