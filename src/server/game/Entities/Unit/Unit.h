...
enum UnitFlags2
{
    ...
    UNIT_FLAG2_CANT_CLICKABLE               = 0x04000000,
};
...
struct CharmInfo
{
    public:
        ...
        void InitCharmCreateSpells(uint32 infoMask = 0);
...
class Unit : public WorldObject
{
    public:
        ...
        Unit* GetNearbyVictim(Unit* exclude = NULL, float dist = NOMINAL_MELEE_RANGE, bool IsInFront = false, bool IsNeutral = false) const;
        ...
        void SetAiAnimKit(uint32 id);

        uint16 GetMaxSkillValueForLevel(Unit const* target = NULL) const { return (target ? getLevelForTarget(target) : getLevel()) * 5; }
        void DealDamageMods(Unit* victim, uint32 &damage, uint32* absorb);
        uint32 DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage = NULL, DamageEffectType damagetype = DIRECT_DAMAGE, SpellSchoolMask damageSchoolMask = SPELL_SCHOOL_MASK_NORMAL, SpellInfo const* spellProto = NULL, bool durabilityLoss = true);
        uint32 CalcStaggerDamage(uint32 damage, SpellSchoolMask damageSchoolMask, SpellInfo const* spellInfo = NULL);
        ...
        void ProcDamageAndSpell(Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procEx, DamageInfo* dmgInfoProc, WeaponAttackType attType = BASE_ATTACK, SpellInfo const* procSpell = NULL, SpellInfo const* procAura = NULL, std::list<uint32>* mSpellModsList = NULL);
        void ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, DamageInfo* dmgInfoProc, SpellInfo const* procAura = NULL, std::list<uint32>* mSpellModsList = NULL);
        ...
        void CalculateMeleeDamage(Unit* victim, float damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType = BASE_ATTACK);
        void DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss);
        void HandleProcExtraAttackFor(Unit* victim);

        void CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, float damage, SpellInfo const* spellInfo, uint32 effectMask, WeaponAttackType attackType = BASE_ATTACK, bool crit = false);
        ...
        void ApplyResilience(const Unit* victim, float * damage, bool isCrit) const;

        float MeleeSpellMissChance(const Unit* victim, WeaponAttackType attType, uint32 spellId, bool checkHitPenalty = true) const;
        ...
        void CastSpellAfterDeley(uint32 deley, Unit* victim, uint32 spellId, bool triggered, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        void CastCustomSpellAfterDeley(uint32 deley, Unit* victim, uint32 spellId, float bp0, float bp1, float bp2, bool triggered, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        ...
        void CastCustomSpell(uint32 spellId, SpellValueMod mod, float value, Unit* Victim = NULL, bool triggered = true, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        void CastCustomSpell(Unit* Victim, uint32 spellId, float const* bp0, float const* bp1, float const* bp2, bool triggered, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        void CastCustomSpell(Unit* Victim, uint32 spellId, float const* bp0, float const* bp1, float const* bp2, float const* bp3, float const* bp4, float const* bp5, bool triggered, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, uint64 originalCaster = 0);
        ...
        */
        ...
        uint64 GetDemonCreatorGUID() const { return GetUInt64Value(UNIT_FIELD_DEMON_CREATOR); }
        ...
        Aura* _TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint32 effMask, Unit* caster, float* baseAmount = NULL, Item* castItem = NULL, uint64 casterGUID = 0);
        ...
        void RemoveAllAurasFromItem(uint64 itemGUID);
        ...
        void SendCreateVisual(Unit* target, float positionX, float positionY, float positionZ, float speed, uint32 visual, uint16 unk1 = 0, uint16 unk2 = 0, bool positionFind = false);
        ...
        void UpdateObjectVisibility(bool forced = true, float customRange = 0.0f);
        ...
        bool   isSpellCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType, float &critChance) const;
        ...
        virtual bool IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const;
                                                            // redefined in Creature
        static bool IsDamageReducedByArmor(SpellSchoolMask damageSchoolMask, SpellInfo const* spellInfo = NULL, uint32 effectMask = 0);
        ...
        uint32 CalcAbsorb(Unit* victim, SpellInfo const* spellProto, uint32 amount);
        ...
        float CalculateSpellDamage(Unit const* target, SpellInfo const* spellProto, uint8 effect_index, float const* basePoints = NULL, Item* m_castitem = NULL, bool lockBasePoints = false) const;
        ...
        float GetTransOffsetX() const { return m_movementInfo.transportPosition.GetPositionX(); }
        float GetTransOffsetY() const { return m_movementInfo.transportPosition.GetPositionY(); }
        float GetTransOffsetZ() const { return m_movementInfo.transportPosition.GetPositionZ(); }
        float GetTransOffsetO() const { return m_movementInfo.transportPosition.GetOrientation(); }
        uint32 GetTransTime()   const { return m_movementInfo.transportMoveTime; }
        int8 GetTransSeat()     const { return m_movementInfo.transportVehicleSeatIndex; }
    ...
    private:
        ...
        bool IsTriggeredAtSpellProcEvent(Unit* victim, SpellInfo const* spellProto, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent, uint8 effect, uint64 castItemGUID);
