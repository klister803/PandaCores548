...
float Unit::GetUnitCriticalChance(WeaponAttackType attackType, const Unit* victim, SpellInfo const* spellProto) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            case BASE_ATTACK:
                crit = GetExactCritPct(PLAYER_CRIT_PERCENTAGE - PLAYER_CRIT_PERCENTAGE);
                break;
            case OFF_ATTACK:
                crit = GetExactCritPct(PLAYER_OFFHAND_CRIT_PERCENTAGE - PLAYER_CRIT_PERCENTAGE);
                break;
            case RANGED_ATTACK:
                crit = GetExactCritPct(PLAYER_RANGED_CRIT_PERCENTAGE - PLAYER_CRIT_PERCENTAGE);
                break;
                // Just for good manner
            default:
                crit = 0.0f;
    ...
    switch (spellProto->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            if (schoolMask == SPELL_SCHOOL_MASK_ALL)
            {
                ...
                {
                    float _crit = GetExactCritPct(PLAYER_SPELL_CRIT_PERCENTAGE1 + i - PLAYER_CRIT_PERCENTAGE);
                    if (_crit > maxCrit)
                        maxCrit = _crit;
                }
            ...
            // For other schools
                ...
                crit_chance = GetExactCritPct(PLAYER_SPELL_CRIT_PERCENTAGE1 + GetFirstSchoolInMask(schoolMask) - PLAYER_CRIT_PERCENTAGE);
