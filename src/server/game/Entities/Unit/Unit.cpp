...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && HasAura(13877) && damage > 0 && (!spellProto || (spellProto && spellProto->Id != 22482)))
    {
        ...
        if (target)
            CastCustomSpell(target, 22482, &bp, NULL, NULL, NULL, NULL, NULL, true);
    }
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->getClass() == CLASS_DEATH_KNIGHT && damage != 0)
    {
        ...
        if (victim->HasAura(114556) && !victim->HasAura(123981) && damage > victim->GetHealth())
        {
            ...
            int32 bp = damage;
            victim->CastCustomSpell(victim, 116888, &bp, NULL, NULL, NULL, NULL, NULL, true);
    ...
    if (GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_ROGUE && damage != 0)
    {
        if (AuraPtr leechingPoison = victim->GetAura(112961))
        {
            if (leechingPoison->GetCaster())
            {
                ...
                {
                    int32 bp = damage / 10;
                    CastCustomSpell(this, 112974, &bp, NULL, NULL, NULL, NULL, NULL, true);
                }
            }
        }
    }
    ...
    if (GetOwner() && GetTypeId() == TYPEID_UNIT && GetEntry() == 29264 && damage > 0)
    {
        ...
        CastCustomSpell(GetOwner(), 58879, &basepoints, NULL, NULL, NULL, NULL, NULL, true, 0, NULLAURA_EFFECT, GetGUID());
    }
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->HasAura(115610) && damage != 0)
    {
        ...
        victim->CastCustomSpell(victim, 115611, &bp, NULL, NULL, NULL, NULL, NULL, true);
    }
        ...
        && spellProto->Id != 130651 && spellProto->Id != 117993) // Don't triggered by Zen Sphere, Spinning Crane Kick, Chi Wave, Chi Burst and Chi Torpedo
    {
        ...
        {
            CastCustomSpell(itr, 117895, &bp, NULL, NULL, NULL, NULL, NULL, true, 0, NULLAURA_EFFECT, GetGUID()); // Eminence - statue
            ...
            if (statueList.size() == 1)
            {
                ...
                if (statue && (statue->isPet() || statue->isGuardian()))
                    if (statue->GetOwner() && statue->GetOwner()->GetGUID() == GetGUID())
                        statue->CastCustomSpell(itr, 117895, &bp, NULL, NULL, NULL, NULL, NULL, true, 0, NULLAURA_EFFECT, GetGUID()); // Eminence - statue
            }
        }
    }
    ...
    if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->getClass() == CLASS_MONK && HasAura(127722) && !spellProto)
    {
        ...
        CastCustomSpell(this, 126890, &bp, NULL, NULL, NULL, NULL, NULL, true, 0, NULLAURA_EFFECT, GetGUID()); // Eminence - statue
        ...
        if (statueList.size() == 1)
        {
            ...
            if (statue && (statue->isPet() || statue->isGuardian()))
                if (statue->GetOwner() && statue->GetOwner()->GetGUID() == GetGUID())
                    statue->CastCustomSpell(statue, 126890, &bp, NULL, NULL, NULL, NULL, NULL, true, 0, NULLAURA_EFFECT, GetGUID()); // Eminence - statue
        }
    }
...
uint32 Unit::CalcStaggerDamage(Player* victim, uint32 damage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->GetSpecializationId(victim->ToPlayer()->GetActiveSpec()) == SPEC_MONK_BREWMASTER && damageSchoolMask == SPELL_SCHOOL_MASK_NORMAL && victim->HasAura(115069) && damage > 0)
    {
        ...
        if (!aurEff)
            victim->CastCustomSpell(victim, spellId, &bp, NULL, NULL, NULL, NULL, NULL, true);
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->getClass() == CLASS_WARLOCK && damageInfo->damage > 0 && victim->HasAura(108446))
    {
        if (victim->ToPlayer()->GetPet() && victim->ToPlayer()->GetPet()->HasAura(108446))
        {
            ...
            victim->ToPlayer()->GetPet()->CastCustomSpell(victim->ToPlayer()->GetPet(), 108451, &bp, NULL, NULL, NULL, NULL, NULL, true); // Soul Link damage
        }
    }
}
...
bool Unit::HandleHasteAuraProc(Unit* victim, uint32 damage, AuraEffectPtr triggeredByAura, SpellInfo const* /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    if (basepoints0)
        CastCustomSpell(target, triggered_spell_id, &basepoints0, NULL, NULL, NULL, NULL, NULL, true, castItem, triggeredByAura);
    else
...
bool Unit::HandleAuraProcOnPowerAmount(Unit* victim, uint32 /*damage*/, AuraEffectPtr triggeredByAura, SpellInfo const *procSpell, uint32 procFlag, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    if (basepoints0)
        CastCustomSpell(target, trigger_spell_id, &basepoints0, NULL, NULL, NULL, NULL, NULL, true, castItem, triggeredByAura);
    else
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffectPtr triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
        ...
        case SPELLFAMILY_WARRIOR:
        {
            switch (dummySpell->Id)
            {
                ...
                case 12292:
                {
                    ...
                    CastCustomSpell(victim, 113344, &bp, NULL, NULL, NULL, NULL, NULL, true);
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 15286:
                {
                    ...
                    CastCustomSpell(this, 15290, &team, &self, NULL, NULL, NULL, NULL, true, castItem, triggeredByAura);
                    return true;                                // no hidden cooldown
                }
            ...
            switch (dummySpell->Id)
            {
                ...
                case 24932:
                {
                   if (triggerAmount <= 0)
                    ...
                    CastCustomSpell(this, 68285, &mana, NULL, NULL, NULL, NULL, NULL, true, castItem, triggeredByAura);
                    break;
                }
                    ...
                    if (!petlist.empty())
                    {
                        for (std::list<Creature*>::const_iterator itr = petlist.begin(); itr != petlist.end(); ++itr)
                        {
                            ...
                            if (pPet->GetOwnerGUID() == GetGUID())
                            {
                                int32 bp0 = damage;
                                int32 bp1 = damage / 10;
                                pPet->CastCustomSpell(victim, 86678, &bp0, &bp1, NULL, NULL, NULL, NULL, true);
                            }
                        }
                    }
                ...
                if (Unit* beaconTarget = triggeredByAura->GetBase()->GetCaster())
                {
                    ...
                    if (beaconTarget->GetAura(53563, victim->GetGUID()))
                    {
                        if (beaconTarget->IsWithinLOSInMap(victim))
                        {
                            ...
                            victim->CastCustomSpell(beaconTarget, 53652, &basepoints0, NULL, NULL, NULL, NULL, NULL, true);
                            return true;
                        }
                    }
                }
            ...
            switch (dummySpell->Id)
            {
                ...
                case 33757:
                {
                    ...
                    for (uint32 i = 0; i < 2; ++i)
                        CastCustomSpell(victim, triggered_spell_id, &basepoints0, NULL, NULL, NULL, NULL, NULL, true, castItem, triggeredByAura);
            ...
            if (dummySpell->Id == 81229 || dummySpell->Id == 51462)
            {
                ...
                if (constAuraEffectPtr runicCorruption = GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 4068, 0))
                {
                    ...
                    if (AuraPtr aur = GetAura(51460))
                        ...
                        CastCustomSpell(this, 51460, &basepoints0, NULL, NULL, NULL, NULL, NULL, true);
                    return true;
                }
            ...
            if (dummySpell->Id == 17619)
            {
                if (procSpell->SpellFamilyName == SPELLFAMILY_POTION)
                {
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
                    {
                        ...
                        CastCustomSpell(this, triggered_spell_id, &basepoints0, NULL, NULL, NULL, NULL, NULL, true, NULL, triggeredByAura);
                    }
                    return true;
                }
            }
    ...
    switch (dummySpell->Id)
    {
        ...
        case 120267:
        {
           ...
           if (!aviableBasepoints && (procFlag & (PROC_FLAG_TAKEN_MELEE_AUTO_ATTACK)))
            ...
            CastCustomSpell(this, triggered_spell_id, &basepoints0, &basepoints0, &basepoints1, NULL, NULL, NULL, true, castItem, triggeredByAura, originalCaster);
            return true;
        }
        default:
            break;
    ...
    if (basepoints0)
        CastCustomSpell(target, triggered_spell_id, &basepoints0, NULL, NULL, NULL, NULL, NULL, true, castItem, triggeredByAura, originalCaster);
    else
        ...
        case SPELLFAMILY_GENERIC:
            switch (dummySpell->Id)
            {
                ...
                case 59906:
                {
                    ...
                    CastCustomSpell(this, 59913, &bp0, NULL, NULL, NULL, NULL, NULL, true);
                    *handled = true;
                    break;
                }
            }
                ...
                *handled = true;
                ...
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
                {
                    if (procSpell->Effects[i].Effect == SPELL_EFFECT_ENERGIZE)
                    {
                        ...
                        CastCustomSpell(this, 54986, NULL, &mana, NULL, NULL, NULL, NULL, true);
                    }
                }
                return true;
            }
            ...
            switch (dummySpell->Id)
            {
                ...
                case 31658:
                {
                    *handled = true;
                    ...
                    CastCustomSpell(this, 67545, &bp0, NULL, NULL, NULL, NULL, NULL, true, NULL, triggeredByAura->GetEffect(EFFECT_0), GetGUID());
                    return true;
                }
            }
            break;
        ...
        case SPELLFAMILY_WARRIOR:
        {
            switch (dummySpell->Id)
            {
                ...
                case 70844:
                {
                    ...
                    CastCustomSpell(this, 70845, &basepoints0, NULL, NULL, NULL, NULL, NULL, true);
                    break;
                }
                // Recklessness
                case 1719:
            ...
            case SPELLFAMILY_GENERIC:
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 64568:             // Blood Reserve
                    {
                        ...
                        {
                            CastCustomSpell(this, 64569, &triggerAmount, NULL, NULL, NULL, NULL, NULL, true);
                            RemoveAura(64568);
                        }
                        return false;
                    }
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth)
{
    ...
    if (unit && unit->GetTypeId() == TYPEID_PLAYER && addhealth != 0 && unit->HasAura(16213))
    {
        ...
        unit->CastCustomSpell(victim, 105284, &bp, NULL, NULL, NULL, NULL, NULL, true);
    }
    ...
    if (unit && victim && addhealth != 0)
    {
        if (unit->GetTypeId() == TYPEID_PLAYER)
        {
            if (unit->HasAura(76669) && unit->getLevel() >= 80)
            {
                ...
                unit->CastCustomSpell(victim, 86273, &bp, NULL, NULL, NULL, NULL, NULL, true);
            }
        }
    }
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && (!spellProto || (spellProto && spellProto->Id != 108451)) && getClass() == CLASS_WARLOCK && tmpDamage > 0 && HasAura(108446))
    {
        if (ToPlayer()->GetPet() && ToPlayer()->GetPet()->HasAura(108446))
        {
            ...
            ToPlayer()->GetPet()->CastCustomSpell(ToPlayer()->GetPet(), 108451, &bp, NULL, NULL, NULL, NULL, NULL, true); // Soul Link damage
        }
    }
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && heal != 0 && HasAura(114052) && spellProto->Id != 114083)
    {
        ...
        for (auto itr : nearbyAllies)
            if (bp > 0)
                CastCustomSpell(itr, 114083, &bp, NULL, NULL, NULL, NULL, NULL, true); // Restorative Mists
    }
...
uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && spellProto->Id != 108447 && getClass() == CLASS_WARLOCK && heal > 0 && HasAura(108446))
    {
        if (ToPlayer()->GetPet() && ToPlayer()->GetPet()->HasAura(108446))
        {
            ...
            ToPlayer()->GetPet()->CastCustomSpell(ToPlayer()->GetPet(), 108447, &bp, NULL, NULL, NULL, NULL, NULL, true); // Soul Link heal
        }
    }
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffectPtr triggeredByAura)
{
    ...
    if (jumps > 0)
    {
        if (Unit* caster = triggeredByAura->GetCaster())
        {
            ...
            {
                CastCustomSpell(target, spellProto->Id, &heal, NULL, NULL, NULL, NULL, NULL, true, NULL, triggeredByAura, caster_guid);
    ...
    // heal
    CastCustomSpell(this, 33110, &heal, NULL, NULL, NULL, NULL, NULL, true, NULL, NULLAURA_EFFECT, caster_guid);
    return true;
