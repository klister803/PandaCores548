...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack, DotaStatsDump *fillIn, DotaStatsDump *use)
{
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask, bool withAp /*= true*/)
{
    int32 DoneAdvertisedBenefit = 0;

    AuraEffectList const& mOverrideSpellPowerAuras = GetAuraEffectsByType(SPELL_AURA_OVERRIDE_SPELL_POWER_BY_AP_PCT);
    if (withAp && !mOverrideSpellPowerAuras.empty())
    {
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (AuraEffectList::const_iterator i = mHealingDoneOfStatPercent.begin(); i != mHealingDoneOfStatPercent.end(); ++i)
        {
            ...
            Stats usedStat = Stats((*i)->GetSpellInfo()->GetEffect((*i)->GetEffIndex(), GetSpawnMode()).MiscValue);
        ...
        if (isVictim)
        {
            if (triggerData.aura->GetSpellInfo()->HasAura(SPELL_AURA_MOD_STEALTH))
            {
                ...
                if (!dmgInfoProc->GetDamage() && procSpell && !procSpell->HasAura(SPELL_AURA_MOD_STUN) && !procSpell->HasAura(SPELL_AURA_MOD_CONFUSE) &&
                    !procSpell->HasAura(SPELL_AURA_MOD_FEAR))
                    continue;
