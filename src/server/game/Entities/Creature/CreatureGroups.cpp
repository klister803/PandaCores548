...
void FormationMgr::AddCreatureToGroup(uint32 groupId, Creature* member)
{
    ...
    {
        TC_LOG_DEBUG("server", "Group found: %u, inserting creature GUID: %u, Group InstanceID %u", groupId, member->GetGUIDLow(), member->GetInstanceId());
        itr->second->AddMember(member);
    }
    //Create new group
    else
    {
        TC_LOG_DEBUG("server", "Group not found: %u. Creating new group.", groupId);
...
{
    TC_LOG_DEBUG("server", "Deleting member pointer to GUID: %u from group %u", group->GetId(), member->GetDBTableGUIDLow());
    ...
    if (group->isEmpty())
    {
        ...
        TC_LOG_DEBUG("server", "Deleting group with InstanceID %u", member->GetInstanceId());
...
void FormationMgr::LoadCreatureFormations()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">>  Loaded 0 creatures in formations. DB table `creature_formations` is empty!");
        return;
    }
    ...
    do
    {
        ...
        // check data correctness
        {
            ...
            {
                TC_LOG_ERROR("sql", "creature_formations table leader guid %u incorrect (not exist)", group_member->leaderGUID);
                delete group_member;
                continue;
            }
            ...
            {
                TC_LOG_ERROR("sql", "creature_formations table member guid %u incorrect (not exist)", memberGUID);
                delete group_member;
                continue;
            }
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u creatures in formations in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
{
    TC_LOG_DEBUG("server", "CreatureGroup::AddMember: Adding unit GUID: %u.", member->GetGUIDLow());

    //Check if it is a leader
    if (member->GetDBTableGUIDLow() == m_groupID || member->GetGUIDLow() == m_groupID)
    {
        TC_LOG_DEBUG("server", "Unit GUID: %u is formation leader. Adding group.", member->GetGUIDLow());
        m_leader = member;
    }
...
void CreatureGroup::MemberAttackStart(Creature* member, Unit* target)
{
    ...
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (m_leader) // avoid crash if leader was killed and reset.
            TC_LOG_DEBUG("server", "GROUP ATTACK: group instance id %u calls member instid %u", m_leader->GetInstanceId(), member->GetInstanceId());
...
void CreatureGroup::FormationReset(bool dismiss)
{
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (itr->first != m_leader && itr->first->isAlive())
        {
            ...
            TC_LOG_DEBUG("server", "Set %s movement for member GUID: %u", dismiss ? "default" : "idle", itr->first->GetGUIDLow());
        }
    }
    m_Formed = !dismiss;
}
