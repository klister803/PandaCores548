...
inline bool CheckDelimiter(std::istringstream& iss, char delimiter, const char* context)
{
    ...
    if (c != delimiter)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): invalid %s link structure ('%c' expected, '%c' found)", iss.str().c_str(), context, delimiter, c);
        return false;
    }
    iss.ignore(1);
    return true;
...
bool ItemChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item entry", iss.str().c_str());
        return false;
    }
    // Validate item
    ...
    if (!_item)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid itemEntry %u in |item command", iss.str().c_str(), itemEntry);
        return false;
    }
    // Validate item's color
    if (_color != ItemQualityColors[_item->Quality])
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): linked item has color %u, but user claims %u", iss.str().c_str(), ItemQualityColors[_item->Quality], _color);
        return false;
    }
    ...
    for (uint8 index = 0; index < propsCount; ++index)
    {
        ...
        if (!ReadInt32(iss, id))
        {
            TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading item property (%u)", iss.str().c_str(), index);
            return false;
        }
        if (id && (index == randomPropertyPosition))
        {
            ...
            if (id > 0)
            {
                ...
                if (!_property)
                {
                    TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid item property id %u in |item command", iss.str().c_str(), id);
                    return false;
                }
            }
            else if (id < 0)
            {
                ...
                if (!_suffix)
                {
                    TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid item suffix id %u in |item command", iss.str().c_str(), -id);
                    return false;
                }
            }
        }
...
bool ItemChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    if (!res)
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): linked item (id: %u) name wasn't found in any localization", context, _item->ItemId);
    return res;
}
...
bool QuestChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading quest entry", iss.str().c_str());
        return false;
    }
    // Validate quest
    ...
    if (!_quest)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): quest template %u not found", iss.str().c_str(), questId);
        return false;
    }
    // Check delimiter
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading quest level", iss.str().c_str());
        return false;
    }
    // Validate quest level
    if (_questLevel >= STRONG_MAX_LEVEL)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): quest level %d is too big", iss.str().c_str(), _questLevel);
        return false;
    }
    return true;
}
...
bool QuestChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    if (!res)
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): linked quest (id: %u) title wasn't found in any localization", context, _quest->GetQuestId());
    return res;
}
...
bool SpellChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading spell entry", iss.str().c_str());
        return false;
    }
    // Validate spell
    ...
    if (!_spell)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u in |spell command", iss.str().c_str(), spellId);
        return false;
    }
    return true;
}
...
bool SpellChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    if (_spell->Attributes & SPELL_ATTR0_TRADESPELL)
    {
        ...
        {
            TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): skill line not found for spell %u", context, _spell->Id);
            return false;
        }
        SkillLineAbilityEntry const* skillInfo = bounds.first->second;
        if (!skillInfo)
        {
            TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): skill line ability not found for spell %u", context, _spell->Id);
            return false;
        }
        SkillLineEntry const* skillLine = sSkillLineStore.LookupEntry(skillInfo->skillId);
        if (!skillLine)
        {
            TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): skill line not found for skill %u", context, skillInfo->skillId);
            return false;
        }
    ...
    TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): linked spell (id: %u) name wasn't found in any localization", context, _spell->Id);
    return false;
}
...
bool AchievementChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading achievement entry", iss.str().c_str());
        return false;
    }
    ...
    if (!_achievement)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid achivement id %u in |achievement command", iss.str().c_str(), achievementId);
        return false;
    }
    // Check delimiter
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): invalid hexadecimal number while reading char's guid", iss.str().c_str());
        return false;
    }
    // Skip progress
    const uint8 propsCount = 8;
    for (uint8 index = 0; index < propsCount; ++index)
    {
        ...
        {
            TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading achievement property (%u)", iss.str().c_str(), index);
            return false;
        }
    }
    return true;
...
bool AchievementChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): linked achievement (id: %u) name wasn't found in any localization", context, _achievement->ID);
    return false;
}
...
bool TradeChatLink::Initialize(std::istringstream& iss)
{
    ...
    if (!_spell)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u in |trade command", iss.str().c_str(), spellId);
        return false;
    }
    // Check delimiter
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading minimum talent level", iss.str().c_str());
        return false;
    }
    // Check delimiter
    if (!CheckDelimiter(iss, DELIMITER, "trade"))
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading maximum talent level", iss.str().c_str());
        return false;
    }
    // Check delimiter
    if (!CheckDelimiter(iss, DELIMITER, "trade"))
    ...
    if (!ReadHex(iss, _guid, 0))
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading achievement's owner guid", iss.str().c_str());
        return false;
    }
...
bool TalentChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading talent entry", iss.str().c_str());
        return false;
    }
    // Validate talent
    ...
    if (!talentInfo)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid talent id %u in |talent command", iss.str().c_str(), _talentId);
        return false;
    }
    ...
    if (!_spell)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u in |trade command", iss.str().c_str(), talentInfo->RankID[0]);
        return false;
    }
    // Delimiter
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading talent rank", iss.str().c_str());
        return false;
    }
    return true;*/
}
...
bool EnchantmentChatLink::Initialize(std::istringstream& iss)
{
    ...
    if (!ReadUInt32(iss, spellId))
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading enchantment spell entry", iss.str().c_str());
        return false;
    }
    // Validate spell
    _spell = sSpellMgr->GetSpellInfo(spellId);
    if (!_spell)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u in |enchant command", iss.str().c_str(), spellId);
        return false;
    }
    return true;
}
...
bool GlyphChatLink::Initialize(std::istringstream& iss)
{
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading slot id", iss.str().c_str());
        return false;
    }
    // Check delimiter
    ...
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading glyph entry", iss.str().c_str());
        return false;
    }
    // Validate glyph
    ...
    if (!_glyph)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid glyph id %u in |glyph command", iss.str().c_str(), glyphId);
        return false;
    }
    ...
    if (!_spell)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid spell id %u in |glyph command", iss.str().c_str(), _glyph->SpellId);
        return false;
    }
    return true;
}
...
bool LinkExtractor::IsValidMessage()
{
    ...
    while (!_iss.eof())
    {
        ...
        {
            TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence aborted unexpectedly", _iss.str().c_str());
            return false;
        }
        ...
        if (_iss.peek() == '\0')
        {
            TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): pipe followed by '\\0'", _iss.str().c_str());
            return false;
        }
        ...
        if (commandChar != PIPE_CHAR)
        {
            ...
            {
                TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): invalid sequence, expected '%c' but got '%c'", _iss.str().c_str(), *validSequenceIterator, commandChar);
                return false;
            }
        }
        else if (validSequence != validSequenceIterator)
        {
            ...
            TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got escaped pipe in sequence", _iss.str().c_str());
            return false;
        }
        ...
        switch (commandChar)
        {
            case 'c':
                ...
                {
                    TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): invalid hexadecimal number while reading color", _iss.str().c_str());
                    return false;
                }
                break;
            case 'H':
                ...
                if (_iss.eof())
                {
                    TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly", _iss.str().c_str());
                    return false;
                }
                ...
                {
                    TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): user sent unsupported link type '%s'", _iss.str().c_str(), buffer);
                    return false;
                }
            ...
            case 'h':
                ...
                if (*validSequenceIterator == 'h')
                {
                    ...
                    if (_iss.get() != '[')
                    {
                        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): link caption doesn't start with '['", _iss.str().c_str());
                        return false;
                    }
            ...
            case '|':
                ...
                break;
            default:
                TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): got invalid command |%c", _iss.str().c_str(), commandChar);
                return false;
        }
    }
    ...
    if (validSequence != validSequenceIterator)
    {
        TC_LOG_DEBUG("chat", "ChatHandler::isValidChatMessage('%s'): EOF in active sequence", _iss.str().c_str());
        return false;
    }
