...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket & recvData)
{
    ...
    if (!joinAsGroup)
    {
        ...
        TC_LOG_DEBUG("bg", "Battleground: player joined queue for bg queue type %u bg type %u: GUID %u, NAME %s", bgQueueTypeId, bgTypeId, _player->GetGUIDLow(), _player->GetName());
    }
    else
    {
        ...
        if (!err)
        {
            TC_LOG_DEBUG("bg", "Battleground: the following players are joining as group:");
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            TC_LOG_DEBUG("bg", "Battleground: player joined queue for bg queue type %u bg type %u: GUID %u, NAME %s", bgQueueTypeId, bgTypeId, member->GetGUIDLow(), member->GetName());
        }
        TC_LOG_DEBUG("bg", "Battleground: group end");
...
void WorldSession::HandleBattlefieldListOpcode(WorldPacket& recvData)
{
    ...
    if (!bl)
    {
        TC_LOG_DEBUG("bg", "BattlegroundHandler: invalid bgtype (%u) with player (Name: %s, GUID: %u) received.", bgTypeId, _player->GetName(), _player->GetGUIDLow());
        return;
    }
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData)
{
    ...
    {
        TC_LOG_ERROR("bg", "HandleBattleFieldPortOpcode queueSlot %u", queueSlot);
        return;
    }

    if (!_player || !_player->InBattlegroundQueue())
    {
        TC_LOG_DEBUG("bg", "BattlegroundHandler: Invalid CMSG_BATTLEFIELD_PORT received from player (Name: %s, GUID: %u), he is not in bg_queue.", _player->GetName(), _player->GetGUIDLow());
        return;
    }
    ...
    {
        TC_LOG_DEBUG("bg", "BattlegroundHandler: invalid queueSlot (%u) received.", queueSlot);
        return;
    }

    if (bgQueueTypeId > MAX_BATTLEGROUND_QUEUE_TYPES)
    {
        TC_LOG_ERROR("bg", "HandleBattleFieldPortOpcode bgQueueTypeId %u", bgQueueTypeId);
        return;
    }
    ...
    {
        TC_LOG_DEBUG("bg", "BattlegroundHandler: invalid bgTypeId (%u) received.", bgTypeId);
        return;
    }
    ...
    Battleground* bg = sBattlegroundMgr->GetBattleground(ginfo.IsInvitedToBGInstanceGUID, (bgTypeId == BATTLEGROUND_AA || bgTypeId == BATTLEGROUND_RATED_10_VS_10) ? BATTLEGROUND_TYPE_NONE : 
        bgTypeId == BATTLEGROUND_RB ? ginfo.NormalBgTypeId : bgTypeId);
    ...
    if (action == 1 && ginfo.JoinType == 0)
    {
        ...
        if (!_player->CanJoinToBattleground())
        {
            ...
            action = 0;
            TC_LOG_DEBUG("bg", "Battleground: player %s (%u) has a deserter debuff, do not port him to battleground!", _player->GetName(), _player->GetGUIDLow());
        }
    ...
    switch (action)
    {
        case 1:                                         // port to battleground
            ...
            TC_LOG_DEBUG("bg", "Battleground: player %s (%u) joined battle for bg %u, bgtype %u, queue type %u.", _player->GetName(), _player->GetGUIDLow(), bg->GetInstanceID(), bg->GetTypeID(), bgQueueTypeId);
            break;
        case 0:                                         // leave queue
            ...
            TC_LOG_DEBUG("bg", "Battleground: player %s (%u) left queue for bgtype %u, queue type %u.", _player->GetName(), _player->GetGUIDLow(), bg->GetTypeID(), bgQueueTypeId);
            break;
        default:
...
void WorldSession::JoinBracket(uint8 slot)
{
    ...
    {
        TC_LOG_DEBUG("bg", "Battleground: leader %s join type %u", _player->GetName(), Jointype);
    ...
    for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        TC_LOG_DEBUG("bg", "Battleground: player joined queue for arena as group bg queue type %u bg type %u: GUID %u, NAME %s", bgQueueTypeId, bgTypeId, member->GetGUIDLow(), member->GetName());
    }
...
{
    TC_LOG_DEBUG("network", "WORLD: CMSG_PERSONAL_RATED_INFO_REQUEST");
