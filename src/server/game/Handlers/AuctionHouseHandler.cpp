...
void WorldSession::HandleAuctionSellItem(WorldPacket & recvData)
{
    ...
    if (!creature)
    {
        TC_LOG_DEBUG("network", "WORLD: HandleAuctionSellItem - Unit (GUID: %u) not found or you can't interact with him.", GUID_LOPART(auctioneerGUID));
        return;
    }
    ...
    {
        TC_LOG_DEBUG("network", "WORLD: HandleAuctionSellItem - Unit (GUID: %u) has wrong faction.", GUID_LOPART(auctioneerGUID));
        return;
    }
    ...
    for (uint32 i = 0; i < itemsCount; ++i)
    {
        ...
        if (itemsCount == 1 && item->GetCount() == stackCount[i])
        {
            ...
            TC_LOG_DEBUG("network", "CMSG_AUCTION_SELL_ITEM: Player %s (guid %d) is selling item %s entry %u (guid %d) to auctioneer %u with count %u with initial bid %u with buyout %u and with time %u (in sec) in auctionhouse %u", _player->GetName(), _player->GetGUIDLow(), item->GetTemplate()->Name1.c_str(), item->GetEntry(), item->GetGUIDLow(), AH->auctioneer, item->GetCount(), bid, buyout, auctionTime, AH->GetHouseId());
        ...
        else // Required stack size of auction does not match to current item stack size, clone item and set correct stack size
        {
            ...
            if(newItem->GetEntry() == 38186)
                TC_LOG_DEBUG("efir", "HandleAuctionSellItem - CloneItem of item %u; finalCount = %u playerGUID %u, itemGUID %u", newItem->GetEntry(), finalCount, _player->GetGUID(), newItem->GetGUID());
...
void WorldSession::HandleAuctionPlaceBid(WorldPacket& recvData)
{
    ...
    if (!creature)
    {
        TC_LOG_DEBUG("network", "WORLD: HandleAuctionPlaceBid - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(auctioneerGUID)));
        return;
    }
    ...
    if (!auction_owner && ObjectMgr::GetPlayerAccountIdByGUID(MAKE_NEW_GUID(auction->owner, 0, HIGHGUID_PLAYER)) == player->GetSession()->GetAccountId())
    {
    ...
    if (price < auction->buyout || auction->buyout == 0)
    {
        ...
        auction->bid = price;
        GetPlayer()->UpdateAchievementCriteria(CRITERIA_TYPE_HIGHEST_AUCTION_BID, price);
    ...
    else
    {
        ...
        GetPlayer()->UpdateAchievementCriteria(CRITERIA_TYPE_HIGHEST_AUCTION_BID, auction->buyout);
...
void WorldSession::HandleAuctionRemoveItem(WorldPacket & recvData)
{
    ...
    if (!creature)
    {
        TC_LOG_DEBUG("network", "WORLD: HandleAuctionRemoveItem - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(auctioneerGUID)));
        return;
    }
...
void WorldSession::HandleAuctionListBidderItems(WorldPacket & recvData)
{
    ...
    if (!creature)
    {
        TC_LOG_DEBUG("network", "WORLD: HandleAuctionListBidderItems - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(auctioneerGUID)));
        recvData.rfinish();
        return;
    }
...
void WorldSession::HandleAuctionListItems(WorldPacket & recvData)
{
    ...
    //TC_LOG_DEBUG("server", "Auctionhouse search (GUID: %u TypeId: %u)",, list from: %u, searchedname: %s, levelmin: %u, levelmax: %u, auctionSlotID: %u, auctionMainCategory: %u, auctionSubCategory: %u, quality: %u, usable: %u",
