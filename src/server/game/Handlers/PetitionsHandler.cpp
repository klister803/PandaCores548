...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket& recvData)
{
    ...
    if (!result)
    {
        TC_LOG_DEBUG("player.items", "Petition %u is not found for player %u %s", GUID_LOPART(petitionguid), GetPlayer()->GetGUIDLow(), GetPlayer()->GetName());
        return;
    }
    Field* fields = result->Fetch();
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket & recvData)
{
    ...
    {
        TC_LOG_ERROR("network", "Petition %u of player %u %s has not supported type %u", GUID_LOPART(petitionGuid), GetPlayer()->GetGUIDLow(), GetPlayer()->GetName(), type);
        return;
    }
...
void WorldSession::SendPetitionShowList(uint64 guid)
{
    ...
    if (!creature->isTabardDesigner())
    {
        TC_LOG_DEBUG("network", "WORLD: HandlePetitionShowListOpcode - Unit (GUID: %u) is not a tabard designer.", uint32(GUID_LOPART(guid)));
        return;
    }
