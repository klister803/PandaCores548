...
void WorldSession::HandleSplitItemOpcode(WorldPacket& recvData)
{
    ...
    if (sObjectMgr->IsPlayerInLogList(GetPlayer()))
    {
        ...
        TC_LOG_DEBUG("dupe", "---HandleSplitItemOpcode; srcbag %u, srcslot %u, dstbag %u, dstslot %u, src %u, dst %u, count %u", srcbag, srcslot, dstbag, dstslot, src, dst, count);
    }
...
void WorldSession::HandleSwapInvItemOpcode(WorldPacket & recvData)
{
    ...
    {
        sObjectMgr->DumpDupeConstant(GetPlayer());
        TC_LOG_DEBUG("dupe", "---HandleSwapInvItemOpcode; src %u, dst %u, srcslot %u, dstslot %u count %u", src, dst, srcslot, dstslot, count);
    }
...
void WorldSession::HandleAutoEquipItemSlotOpcode(WorldPacket& recvData)
{
    ...
    {
        sObjectMgr->DumpDupeConstant(GetPlayer());
        TC_LOG_DEBUG("dupe", "---WorldSession::HandleAutoEquipItemSlotOpcode dstpos %u entry %u", dstpos, item->GetEntry());
    }
...
void WorldSession::HandleSwapItem(WorldPacket& recvData)
{
    ...
    {
        sObjectMgr->DumpDupeConstant(GetPlayer());
        TC_LOG_DEBUG("dupe", "---HandleSwapItem; src %u dst %u", src, dst);
    }
...
void WorldSession::HandleAutoEquipItemOpcode(WorldPacket & recvData)
{
    ...
    if(pSrcItem->GetEntry() == 38186)
        TC_LOG_DEBUG("efir", "HandleAutoEquipItemOpcode - item %u; count = %u playerGUID %u, itemGUID %u srcbag %u srcslot %u",
    ...
    else                                                    // have currently equipped item, not simple case
    {
        ...
        if(pDstItem->GetEntry() == 38186)
            TC_LOG_DEBUG("efir", "HandleAutoEquipItemOpcode - item %u; count = %u playerGUID %u, itemGUID %u dstbag %u dstslot %u",
...
void WorldSession::SendItemSparseDb2Reply(uint32 entry)
{
    ...
    for (uint32 x = 0; x < MAX_ITEM_PROTO_STATS; ++x)
        buff << float(proto->ItemStat[x].ItemStatUnk2);
...
void WorldSession::HandleBuybackItem(WorldPacket& recvData)
{
    ...
    {
        sObjectMgr->DumpDupeConstant(GetPlayer());
        TC_LOG_DEBUG("dupe", "---HandleBuybackItem; vendor: %llu; slot %u", GUID_LOPART(vendorguid), slot);
    }
    ...
    if (pItem)
    {
        ...
        if(pItem->GetEntry() == 38186)
            TC_LOG_DEBUG("efir", "HandleBuyItemInSlotOpcode item %u; count = %u playerGUID %u vendorguid %u", pItem->GetEntry(), pItem->GetCount(), _player->GetGUID(), creature->GetGUID());
        ...
        if (msg == EQUIP_ERR_OK)
        {
            ...
            _player->UpdateAchievementCriteria(CRITERIA_TYPE_RECEIVE_EPIC_ITEM, pItem->GetEntry(), pItem->GetCount());
...
void WorldSession::HandleBuyItemInSlotOpcode(WorldPacket & recvData)
{
    ...
    {
        sObjectMgr->DumpDupeConstant(GetPlayer());
        TC_LOG_DEBUG("dupe", "---HandleBuyItemInSlotOpcode item %u; count = %u playerGUID %u vendorguid %u", item, count, _player->GetGUID(), vendorguid);
    }

    if(item == 38186)
        TC_LOG_DEBUG("efir", "HandleBuyItemInSlotOpcode item %u; count = %u playerGUID %u vendorguid %u", item, count, _player->GetGUID(), vendorguid);
...
void WorldSession::AddToQueueBuyItemOpcode(WorldPacket& recvData)
{
    ...
    {
        sObjectMgr->DumpDupeConstant(GetPlayer());
        TC_LOG_DEBUG("dupe", "---HandleBuyItemOpcode; vendor: %llu; item: %u; count: %u;", GUID_LOPART(vendorguid), item, count);
    }

    if(item == 38186)
        TC_LOG_DEBUG("efir", "HandleBuyItemOpcode item %u; count = %u playerGUID %u", item, count, _player->GetGUID());
...
void WorldSession::SendListInventory(uint64 vendorGuid, uint32 entry)
{
    ...
    for (uint32 slot = 0; slot < rawItemCount; ++slot)
    {
        ...
        if (vendorItem->Type == ITEM_VENDOR_TYPE_ITEM)
        {
            ...
            itemsData << uint32(itemTemplate->MaxDurability ? itemTemplate->MaxDurability : -1);
...
void WorldSession::HandleAutoStoreBagItemOpcode(WorldPacket& recvPacket)
{
    ...
    if(pItem->GetEntry() == 38186)
        TC_LOG_DEBUG("efir", "HandleAutoStoreBagItemOpcode - item %u; count = %u playerGUID %u, itemGUID %u dstbag %u srcslot %u srcbag %u",
...
void WorldSession::HandleBuyBankSlotOpcode(WorldPacket& recvPacket)
{
    ...
    */
    ...
    _player->UpdateAchievementCriteria(CRITERIA_TYPE_BUY_BANK_SLOT);
}
...
void WorldSession::HandleWrapItemOpcode(WorldPacket& recvData)
{
    ...
    if (count != 2)
    {
        TC_LOG_DEBUG("network", "WRAP: data count != 2 (%u)", count);
        recvData.rfinish();
        return;
    }
...
void WorldSession::HandleTransmogrifyItems(WorldPacket& recvData)
{
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        if (itemGuids[i])
        {
            ...
            {
                //TC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems Cheats - Player (GUID: %u, name: %s) tried to transmogrify with an invalid item (lowguid: %u).", player->GetGUIDLow(), player->GetName(), GUID_LOPART(itemGuids[i]));
                return;
            }
        }
        ...
        //{
        //    TC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) can't equip the item to be transmogrified (slot: %u, entry: %u).", player->GetGUIDLow(), player->GetName(), slots[i], itemTransmogrified->GetEntry());
        //    return;
        //}
        //
        ...
        //{
        //    TC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) can't equip the transmogrifier item (slot: %u, entry: %u).", player->GetGUIDLow(), player->GetName(), slots[i], itemTransmogrifier->GetEntry());
        //    return;
        //}
        ...
        else
        {
            ...
            {
                TC_LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) failed CanTransmogrifyItem", player->GetGUIDLow(), player->GetName());
                return;
            }
...
void WorldSession::HandleUpgradeItem(WorldPacket& recvData)
{
    ...
    TC_LOG_DEBUG("network", "WORLD: Received CMSG_UPGRADE_ITEM upgradeId %u, unk1 %u, unk2 %u", upgradeId, unk1, unk2);
    ...
    {
        TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItem - Unit (GUID: %u) not found or player can't interact with it.", GUID_LOPART(npcGuid));
        return;
    }
    ...
    if (!item)
    {
        TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItem - Can't find item (GUID: %u).", GUID_LOPART(itemGuid));
        return;
    }
    ...
    if (!newUpgrade)
    {
        TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItem - Can't find upgrade id %u for item %u (GUID: %u).",
    ...
    {
        TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItem - Previous item upgrade id mismatch: %u should be %u. Item id %u (GUID: %u).",
    ...
    if (uint32 reqCur = newUpgrade->currencyReqId)
    {
        ...
        {
            TC_LOG_DEBUG("network", "WORLD: HandleUpgradeItem - insufficient currency: upgrade id %u Item id %u (GUID: %u).",
                upgradeId, item->GetEntry(), GUID_LOPART(itemGuid));
