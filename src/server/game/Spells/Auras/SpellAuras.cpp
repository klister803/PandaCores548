...
_flags(AFLAG_NONE), _effectsToApply(effMask), _needClientUpdate(false), _effectMask(NULL)
{
    ...
    if (GetBase()->CanBeSentToClient())
    {
        ...
        if (slot < MAX_AURAS)
        {
            ...
            SetNeedClientUpdate();
            TC_LOG_DEBUG("spell", "Aura: %u Effect: %d put to unit visible auras slot: %u", GetBase()->GetId(), GetEffectMask(), slot);
        }
        else
            TC_LOG_DEBUG("spell", "Aura: %u Effect: %d could not find empty unit visible slot", GetBase()->GetId(), GetEffectMask());
    }
...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    TC_LOG_DEBUG("spell", "AuraApplication::_HandleEffect: GetId %i, GetAuraType %u, apply: %u: amount: %f, m_send_baseAmount: %f, effIndex: %i",
    GetBase()->GetId(), aurEff->GetAuraType(), apply, aurEff->GetAmount(), aurEff->GetBaseSendAmount(), effIndex);
...
Aura* Aura::TryRefreshStackOrCreate(SpellInfo const* spellproto, uint32 tryEffMask, WorldObject* owner, Unit* caster, float* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, uint64 casterGUID /*= 0*/, bool* refresh /*= NULL*/, uint16 stackAmount, Spell* spell)
{
    ASSERT(spellproto);
    ASSERT(owner);
...
Aura* Aura::TryCreate(SpellInfo const* spellproto, uint32 tryEffMask, WorldObject* owner, Unit* caster, float* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, uint64 casterGUID /*= 0*/)
{
    ASSERT(spellproto);
    ASSERT(owner);
    ASSERT(caster || casterGUID);
...
Aura* Aura::Create(SpellInfo const* spellproto, uint32 effMask, WorldObject* owner, Unit* caster, float* baseAmount, Item* castItem, uint64 casterGUID, uint16 stackAmount, Spell* spell)
{
    ASSERT(effMask);
    ASSERT(spellproto);
    ASSERT(owner);
    ...
    if(spellproto->IsSingleTarget(caster))
    {
        ...
        for (Unit::AuraList::iterator itr = scAuras.begin(); itr != scAuras.end();)
        {
            ...
            {
                //test code
                /*TC_LOG_DEBUG("spell", "Aura* Aura::Create aura %u, GetCasterGUID %u", (*itr)->GetId(), caster->GetGUID());
...
void Aura::_UnapplyForTarget(Unit* target, Unit* caster, AuraApplication * auraApp)
{
    ...
    {
        TC_LOG_ERROR("spell", "Aura::_UnapplyForTarget, target:%u, caster:%u, spell:%u was not found in owners application map!",
    ...
    if (caster) // note: item based cooldowns and cooldown spell mods with charges ignored (unknown existed cases)
        if (Player* plr = caster->ToPlayer())
            ...
            {
                if ((m_spellInfo->GetRecoveryTime() / IN_MILLISECONDS) > uint32(plr->GetSpellCooldownDelay(m_spellInfo->Id)))
                    return;

                plr->SendCooldownEvent(m_spellInfo);
            }
}
    ...
    for (std::map<Unit*, uint32>::iterator itr = targets.begin(); itr!= targets.end();)
    {
        ...
        else
        {
            ...
            if (!GetOwner()->IsSelfOrInSameMap(itr->first))
            {
                ...
                TC_LOG_FATAL("spell", "Aura %u: Owner %s (map %u) is not in the same map as target %s (map %u).", GetSpellInfo()->Id,
...
{
    if (IsUsingCharges() || (m_spellInfo->ProcFlags & (PROC_FLAG_DONE_SPELL_DMG_POS_NEG)))
    {
                    ...
                    case 12043: // Presence of Mind
                        ...
                        if (AuraEffect const* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 2120, 0))
                        {
                            ...
                            switch (aurEff->GetId())
                            {
                                ...
                                default:
                                    TC_LOG_ERROR("spell", "Aura::HandleAuraSpecificMods: Unknown rank of Arcane Potency (%d) found", aurEff->GetId());
                            }
                            if (spellId)
                ...
                if (GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400)
                {
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 98, 0))
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            default:
                                TC_LOG_ERROR("spell", "Aura::HandleAuraSpecificMods: Unknown rank of Improved Fear (%d) found", aurEff->GetId());
                        }
                        if (spellId)
...
void Aura::LoadScripts()
{
    ...
    for (std::list<AuraScript*>::iterator itr = m_loadedScripts.begin(); itr != m_loadedScripts.end();)
    {
        ...
        TC_LOG_DEBUG("spell", "Aura::LoadScripts: Script `%s` for aura `%u` is loaded now", (*itr)->_GetScriptName()->c_str(), m_spellInfo->Id);
        (*itr)->Register();
        ++itr;
    }
}
...
void Aura::CallScriptEffectChangeTickDamageHandlers(AuraEffect const* aurEff, float & amount, Unit* target, bool crit)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_CALC_AMOUNT);
...
UnitAura::UnitAura(SpellInfo const* spellproto, uint32 effMask, WorldObject* owner, Unit* caster, float *baseAmount, Item* castItem, uint64 casterGUID, uint16 stackAmount)
...
DynObjAura::DynObjAura(SpellInfo const* spellproto, uint32 effMask, WorldObject* owner, Unit* caster, float *baseAmount, Item* castItem, uint64 casterGUID, uint16 stackAmount)
    : Aura(spellproto, owner, caster, castItem, casterGUID, stackAmount)
{
}
