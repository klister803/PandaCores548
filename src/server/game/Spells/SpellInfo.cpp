...
SpellImplicitTargetInfo::StaticData  SpellImplicitTargetInfo::_data[TOTAL_SPELL_TARGETS] =
{
    ...
    {TARGET_OBJECT_TYPE_OBJ_AND_DEST, TARGET_REFERENCE_TYPE_DEST,   TARGET_SELECT_CATEGORY_DEFAULT, TARGET_CHECK_ENTRY,    TARGET_DIR_NONE},        // 46 TARGET_DEST_NEARBY_ENTRY
    ...
    {TARGET_OBJECT_TYPE_DEST, TARGET_REFERENCE_TYPE_CASTER, TARGET_SELECT_CATEGORY_DEFAULT, TARGET_CHECK_DEFAULT,  TARGET_DIR_FRONT},       // 125 TARGET_DEST_MAX_RANGE
...
float SpellEffectInfo::CalcValue(Unit const* caster, float const* bp, Unit const* target, Item* m_castItem, bool lockBasePoints) const
{
    ...
    else
    {
        if (caster)
        {
            ...
            if (!_spellInfo->SpellLevel && _spellInfo->BaseLevel > 0)
                level -= int32(_spellInfo->BaseLevel);
            else
...
SpellInfo::SpellInfo(SpellEntry const* spellEntry)
{
   ...
   // SpellPowerId = spellEntry->SpellPowerId;
    ...
    NullEffect = SpellEffectInfo(spellEntry, this, MAX_SPELL_EFFECTS, nullptr);

    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
    {
        bool insert = false;
        for (uint8 difficulty = NONE_DIFFICULTY; difficulty < MAX_DIFFICULTY; ++difficulty)
        {
            if (SpellEffectEntry const* _effect = spellEntry->GetSpellEffect(i, difficulty))
            {
                ...
                EffectDiffycultyMask |= 1 << difficulty;

                if (difficulty == NONE_DIFFICULTY)
                {
...
SpellEffectInfo* SpellInfo::GetDifficultyEffectInfo(uint8 effect, uint8 difficulty)
{
    ...
    if (difficulty && (EffectDiffycultyMask & (1 << difficulty)))
    {
...
bool SpellInfo::IsTargetingValidForVengeance() const
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (Effects[i]->IsEffect())
            if (!Effects[i]->IsEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA) && !Effects[i]->IsAreaAuraEffect())
                return true;
    return false;
}
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player const* player) const
{
    ...
    switch (Id)
    {
        ...
        case SPELL_SET_FACTION_ALLIANCE:
        case SPELL_SET_FACTION_HORDE:
        {
            MapEntry const* mapEntry = sMapStore.LookupEntry(map_id);
            if (!mapEntry)
                return SPELL_FAILED_INCORRECT_AREA;
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
    ...
    if (unitTarget)
    {
        ...
        if (AttributesEx5 & SPELL_ATTR5_UNK8)
    ...
    if (!(GeneralTargets & TARGET_UNIT_MASTER) && !caster->IsVehicle() && !unitTarget->IsMagnetSpellTarget())
    {
...
uint32 SpellInfo::CalcCastTime(Unit* caster, Spell* spell) const
{
    ...
    if (caster && CastTimeMax > 0)
    {
        ...
        if (CastTimeMaxLevel > caster->getLevel())
            castTime = CastTimeMin + caster->getLevel() - 1 * (CastTimeMax - CastTimeMin) / (CastTimeMaxLevel - 1);
    }
