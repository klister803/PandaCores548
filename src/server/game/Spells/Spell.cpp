...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (m_caster != unit)
    {
        ...
        if (m_caster->_IsValidAttackTarget(unit, m_spellInfo))
        {
            ...
            if (m_spellInfo->HasAura(SPELL_AURA_MOD_STUN) || m_spellInfo->HasAura(SPELL_AURA_MOD_CONFUSE) ||
                m_spellInfo->HasAura(SPELL_AURA_MOD_FEAR) || m_spellInfo->HasAura(SPELL_AURA_MOD_ROOT))
            {
                if (!unit->HasAura(11327))
                {
                    unit->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
                }
            }
            else if ((m_spellInfo->AttributesCu & SPELL_ATTR0_CU_AURA_CC) && unit->IsControlledByPlayer() && 
                !(m_spellInfo->SpellFamilyFlags[0] & 0x20000 || m_spellInfo->SpellFamilyFlags[1] & 0x10000000))
            {
