...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (m_healing > 0)
    {
        ...
        {
            DamageInfo dmgInfoProc = DamageInfo(m_caster, unitTarget, addhealth, m_spellInfo, SpellSchoolMask(m_spellInfo->SchoolMask), SPELL_DIRECT_DAMAGE, m_healBeforeTake);
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CAST_IN_PROGRESS) && m_caster->IsNonMeleeSpellCasted(false, true, true, m_spellInfo->Id == 75) && m_cast_count)
    {
    ...
    if (!m_powerCost)// Fill cost data (not use power for item casts
...
void Spell::cast(bool skipCheck)
{
    ...
    if (((m_spellInfo->Speed > 0.0f || m_delayMoment) && !m_spellInfo->IsChanneled() && !m_spellInfo->IsNonNeedDelay() && m_spellInfo->Id != 114157)  || m_spellInfo->AttributesEx4 & SPELL_ATTR4_UNK4 || m_spellInfo->Id == 54957 || m_spellInfo->Id == 115945)
    {
...
void Spell::SendSpellCooldown()
{
    ...
    if (_triggeredCastFlags & TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD && !(AttributesCustom & SPELL_ATTR0_DISABLED_WHILE_ACTIVE))
        return;

    // set infinity cooldown state for spells
    if (AttributesCustom & SPELL_ATTR0_DISABLED_WHILE_ACTIVE)
    {
        Item* castItem = m_castItemGUID ? _player->GetItemByGuid(m_castItemGUID) : NULL;
        _player->AddSpellAndCategoryCooldowns(m_spellInfo, castItem ? castItem->GetEntry() : 0, this, true);
    }
    else
...
void Spell::finish(bool ok)
{
    ...
    {
        bool found = false;
        if (Unit::AuraEffectList const* vIgnoreReset = m_caster->GetAuraEffectsByType(SPELL_AURA_IGNORE_MELEE_RESET))
...
void Spell::SendLogExecute()
{
    ...
    m_caster->SendMessageToSetInRange(&data, m_caster->GetVisibilityCombatLog(), true);
}
...
void Spell::TakeRunePower(bool didHit)
{
    ...
    if (didHit)
        ...
        {
            if (Unit::AuraEffectList const* ModRuneRegen = player->GetAuraEffectsByType(SPELL_AURA_MOD_RUNE_REGEN_SPEED))
...
SpellCastResult Spell::CheckCasterAuras() const
{
    ...
    else if (unitflag & UNIT_FLAG_PACIFIED)
    {
        ...
        if (Unit::AuraEffectList const* pacifAuras = m_caster->GetAuraEffectsByType(SPELL_AURA_MOD_PACIFY_SILENCE))
            ...
            case SPELL_EFFECT_CREATE_ITEM_3:
            {
                if(ItemTemplate const* itemProto = m_targets.GetItemTarget()->GetTemplate())
                {
                    if(p_caster->CanUseItem(itemProto) != EQUIP_ERR_OK)
                        return SPELL_FAILED_BAD_TARGETS;
                    if (!(itemProto->Flags & ITEM_FLAG_USES_RESOURCES) || itemProto->ItemLevel != 496 || itemProto->ItemId == 104347) //Timeless Curio cannot be target
                        return SPELL_FAILED_BAD_TARGETS;
                }
                else
                    return SPELL_FAILED_BAD_TARGETS;
