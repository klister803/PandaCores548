...
void LoadWeatherData()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 weather definitions. DB table `game_weather` is empty.");
    ...
    do
    {
        ...
        for (uint8 season = 0; season < WEATHER_SEASONS; ++season)
        {
            ...
            if (wzc.data[season].rainChance > 100)
            {
                ...
                TC_LOG_ERROR("sql", "Weather for zone %u season %u has wrong rain chance > 100%%", zone_id, season);
            }

            if (wzc.data[season].snowChance > 100)
            {
                wzc.data[season].snowChance = 25;
                TC_LOG_ERROR("sql", "Weather for zone %u season %u has wrong snow chance > 100%%", zone_id, season);
            }

            if (wzc.data[season].stormChance > 100)
            {
                wzc.data[season].stormChance = 25;
                TC_LOG_ERROR("sql", "Weather for zone %u season %u has wrong storm chance > 100%%", zone_id, season);
            }
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u weather definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
