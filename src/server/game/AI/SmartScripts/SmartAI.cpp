...
WayPoint* SmartAI::GetNextWayPoint()
{
    ...
    if (itr != mWayPoints->end())
    {
        ...
        {
            TC_LOG_ERROR("server", "SmartAI::GetNextWayPoint: Got not expected waypoint id %u, expected %u", mLastWP->id, mCurrentWPID);
        }
        return (*itr).second;
    }
    return NULL;
...
void SmartAI::StartPath(bool run, uint32 path, bool repeat, Unit* /*invoker*/)
{
    ...
    {
        TC_LOG_ERROR("server", "SmartAI::StartPath: Creature entry %u wanted to start waypoint movement while in combat, ignoring.", me->GetEntry());
        return;
    }
...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    {
        TC_LOG_ERROR("server", "SmartAI::StartPath: Creature entry %u wanted to pause waypoint movement while already paused, ignoring.", me->GetEntry());
        return;
    }
...
{
    TC_LOG_DEBUG("smartai", "SmartGameObjectAI::GossipHello");
...
class SmartTrigger : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* trigger, bool apply)
        {
            ...
            TC_LOG_DEBUG("smartai", "AreaTrigger %u is using SmartTrigger script", trigger->id);
            SmartScript script;
