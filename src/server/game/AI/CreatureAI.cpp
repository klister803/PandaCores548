...
void CreatureAI::ZoneTalk(uint8 id, uint64 WhisperGuid)
{
...
void CreatureAI::DoZoneInCombat(Creature* creature /*= NULL*/, float maxRangeToNearestTarget /* = 50.0f*/)
{
    ...
    {
        TC_LOG_ERROR("server", "DoZoneInCombat call for map that isn't an instance (creature entry = %d)", creature->GetTypeId() == TYPEID_UNIT ? creature->ToCreature()->GetEntry() : 0);
        return;
    }
    ...
    if (!creature->HasReactState(REACT_PASSIVE) && !creature->getVictim())
    {
        TC_LOG_ERROR("server", "DoZoneInCombat called for creature that has empty threat list (creature entry = %u)", creature->GetEntry());
        return;
    }
...
void CreatureAI::EnterEvadeMode()
{
    ...
    TC_LOG_DEBUG("server", "Creature %u enters evade mode.", me->GetEntry());
