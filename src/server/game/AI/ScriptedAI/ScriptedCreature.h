...
struct ScriptedAI : public CreatureAI
{
    ...
    float GetHealthPct(uint32 damage) const { return damage > me->GetHealth() ? 0.0f : 100.0f * float(float(me->GetHealth() - damage) / me->GetMaxHealth()); }
    float GetHealthPctWithHeal(uint32 heal) const { return 100.0f * float(float(me->GetHealth() + heal) / me->GetMaxHealth()); }
