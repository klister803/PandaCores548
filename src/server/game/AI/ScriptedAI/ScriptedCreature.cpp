...
SpellInfo const* ScriptedAI::SelectSpell(Unit* target, uint32 school, uint32 mechanic, SelectTargetType targets, uint32 powerCostMin, uint32 powerCostMax, float rangeMin, float rangeMax, SelectEffect effects)
{
    ...
    for (uint32 i = 0; i < CREATURE_MAX_SPELLS; i++)
    {
        ...
        if (powerCostMin && tempSpell->PowerCost < powerCostMin)
            continue;

        if (powerCostMax && tempSpell->PowerCost > powerCostMax)
            continue;

        //Continue if we don't have the mana to actually cast this spell
        if (tempSpell->PowerCost > (uint32)me->GetPower(Powers(tempSpell->PowerType)))
            continue;
...
bool ScriptedAI::EnterEvadeIfOutOfCombatArea(uint32 const diff)
{
    ...
    switch (me->GetEntry())
    {
        ...
        case NPC_SUN_TENDERHEART:
            if (x > 1162.0f && x < 1258.0f && y > 984.0f && y < 1080.0f && z > 410.0f && z < 425.0f)
                return false;
            break;
        default: // For most of creatures that certain area is their home area.
            TC_LOG_DEBUG("server", "TSCR: EnterEvadeIfOutOfCombatArea used for creature entry %u, but does not have any definition. Using the default one.", me->GetEntry());
