...
void FollowerAI::EnterEvadeMode()
{
    ...
    {
        TC_LOG_DEBUG("server", "FollowerAI left combat, returning to CombatStartPosition.");
...
void FollowerAI::UpdateAI(uint32 uiDiff)
{
    if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !me->getVictim())
    {
        if (m_uiUpdateFollowTimer <= uiDiff)
        {
            ...
            {
                TC_LOG_DEBUG("server", "FollowerAI is set completed, despawns.");
            ...
            if (Player* player = GetLeaderForFollower())
            {
                ...
                {
                    TC_LOG_DEBUG("server", "FollowerAI is returning to leader.");
            ...
            {
                TC_LOG_DEBUG("server", "FollowerAI failed because player/group was to far away or not found");
                me->DespawnOrUnsummon();
                return;
            }
...
void FollowerAI::StartFollow(Player* player, uint32 factionForFollower, const Quest* quest)
{
    if (me->getVictim())
    {
        TC_LOG_DEBUG("server", "FollowerAI attempt to StartFollow while in combat.");
        return;
    }

    if (HasFollowState(STATE_FOLLOW_INPROGRESS))
    {
        TC_LOG_ERROR("server", "FollowerAI attempt to StartFollow while already following.");
        return;
    }
    ...
    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        ...
        TC_LOG_DEBUG("server", "FollowerAI start with WAYPOINT_MOTION_TYPE, set to MoveIdle.");
    }
    ...
    TC_LOG_DEBUG("server", "FollowerAI start follow %s (GUID " UI64FMTD ")", player->GetName(), m_uiLeaderGUID);
}
...
Player* FollowerAI::GetLeaderForFollower()
{
        ...
        else
        {
            if (Group* group = player->GetGroup())
            {
                for (GroupReference* groupRef = group->GetFirstMember(); groupRef != NULL; groupRef = groupRef->next())
                {
                    ...
                    {
                        TC_LOG_DEBUG("server", "FollowerAI GetLeader changed and returned new leader.");
    ...
    TC_LOG_DEBUG("server", "FollowerAI GetLeader can not find suitable leader.");
    return NULL;
}
