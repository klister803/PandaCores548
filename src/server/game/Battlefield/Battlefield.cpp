...
void Battlefield::InitStalker(uint32 entry, float x, float y, float z, float o)
{
    if (Creature* creature = SpawnCreature(entry, x, y, z, o, TEAM_NEUTRAL))
        ...
        TC_LOG_ERROR("bg.wg", "Battlefield::InitStalker: could not spawn Stalker (Creature entry %u), zone messeges will be un-available", entry);
}
...
BfGraveyard* Battlefield::GetGraveyardById(uint32 id)
{
    ...
    TC_LOG_ERROR("bg.wg", "Battlefield::GetGraveyardById Id:%u cant be found", id);
    return NULL;
}
...
void BfGraveyard::SetSpirit(Creature* spirit, TeamId team)
{
    if (!spirit)
    {
        TC_LOG_ERROR("bg.wg", "BfGraveyard::SetSpirit: Invalid Spirit.");
        return;
    }
...
Creature* Battlefield::SpawnCreature(uint32 entry, float x, float y, float z, float o, TeamId team)
{
    ...
    if (!map)
    {
        TC_LOG_ERROR("bg.wg", "Battlefield::SpawnCreature: Can't create creature entry: %u map not found", entry);
        return 0;
    }
    ...
    {
        TC_LOG_ERROR("bg.wg", "Battlefield::SpawnCreature: Can't create creature entry: %u", entry);
        delete creature;
        return NULL;
    }
    ...
    if (!cinfo)
    {
        TC_LOG_ERROR("bg.wg", "Battlefield::SpawnCreature: entry %u does not exist.", entry);
        return NULL;
    }
...
GameObject* Battlefield::SpawnGameObject(uint32 entry, float x, float y, float z, float o)
{
    ...
    {
        TC_LOG_ERROR("bg.wg", "Battlefield::SpawnGameObject: Gameobject template %u not found in database! Battlefield not created!", entry);
        TC_LOG_ERROR("bg.wg", "Battlefield::SpawnGameObject: Cannot create gameobject template %u! Battlefield not created!", entry);
        delete go;
        return NULL;
    }
...
bool BfCapturePoint::SetCapturePointData(GameObject* capturePoint)
{
    ...
    TC_LOG_DEBUG("bg.wg", "Creating capture point %u", capturePoint->GetEntry());
    ...
    {
        TC_LOG_ERROR("server", "OutdoorPvP: GO %u is not capture point!", capturePoint->GetEntry());
        return false;
    }
...
bool BfCapturePoint::Update(uint32 diff)
{
    ...
    {
        //TC_LOG_ERROR("bg.wg", "%u->%u", m_OldState, m_State);
        if (oldTeam != m_team)
        {
            ChangeTeam(oldTeam);
