...
void OutdoorPvPMgr::InitOutdoorPvP()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", ">> Loaded 0 outdoor PvP definitions. DB table `outdoorpvp_template` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql", "Invalid OutdoorPvPTypes value %u in outdoorpvp_template; skipped.", typeId);
            continue;
        }
    ...
    for (uint8 i = 1; i < MAX_OUTDOORPVP_TYPES; ++i)
    {
        ...
        {
            TC_LOG_ERROR("sql", "Could not initialize OutdoorPvP object for type ID %u; no entry in database.", uint32(i));
            continue;
        }
    ...
    TC_LOG_INFO("server", ">> Loaded %u outdoor PvP definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
