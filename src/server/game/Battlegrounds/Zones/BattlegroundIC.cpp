...
void BattlegroundIC::DoAction(uint32 action, uint64 var)
{
    ...
    player->m_movementInfo.transportPosition.m_positionX = TransportMovementInfo.GetPositionX();
    player->m_movementInfo.transportPosition.m_positionY = TransportMovementInfo.GetPositionY();
    player->m_movementInfo.transportPosition.m_positionZ = TransportMovementInfo.GetPositionZ();
    player->m_movementInfo.transportGUID = (player->GetBGTeamId() == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde)->GetGUID();
...
void BattlegroundIC::StartingEventOpenDoors()
{
    ...
    for (uint8 i = 0; i < MAX_FORTRESS_TELEPORTERS_SPAWNS; i++)
    {
        if (!AddObject(BG_IC_Teleporters[i].type, BG_IC_Teleporters[i].entry,
            ...
            TC_LOG_ERROR("bg", "Isle of Conquest | Starting Event Open Doors: There was an error spawning gameobject %u", BG_IC_Teleporters[i].entry);
    }
}
...
{
    FillInitialWorldState(data, BG_IC_ALLIANCE_RENFORT_SET, 1);
    FillInitialWorldState(data, BG_IC_HORDE_RENFORT_SET, 1);
    FillInitialWorldState(data, BG_IC_ALLIANCE_RENFORT, factionReinforcements[TEAM_ALLIANCE]);
    FillInitialWorldState(data, BG_IC_HORDE_RENFORT, factionReinforcements[TEAM_HORDE]);
    ...
    for (uint8 i = 0; i < MAX_FORTRESS_GATES_SPAWNS; i++)
    {
        ...
        FillInitialWorldState(data, uws, 1);
    }

    for (uint8 i = 0; i < MAX_NODE_TYPES; i++)
        FillInitialWorldState(data, nodePoint[i].worldStates[nodePoint[i].nodeState], 1);
}
...
bool BattlegroundIC::SetupBattleground()
{
    for (uint8 i = 0; i < MAX_NORMAL_GAMEOBJECTS_SPAWNS; i++)
    {
        if (!AddObject(BG_IC_ObjSpawnlocs[i].type, BG_IC_ObjSpawnlocs[i].entry,
            ...
            0, 0, 0, 0, RESPAWN_ONE_DAY))
        {
            TC_LOG_ERROR("bg", "Isle of Conquest: There was an error spawning gameobject %u", BG_IC_ObjSpawnlocs[i].entry);
            return false;
        }
    }
    ...
    for (uint8 i = 2; i < MAX_NORMAL_NPCS_SPAWNS; i++)
    {
        if (!AddCreature(BG_IC_NpcSpawnlocs[i].entry, BG_IC_NpcSpawnlocs[i].type, BG_IC_NpcSpawnlocs[i].team,
            ...
            RESPAWN_ONE_DAY))
        {
            TC_LOG_ERROR("bg", "Isle of Conquest: There was an error spawning creature %u", BG_IC_NpcSpawnlocs[i].entry);
            return false;
        }
    }
    ...
    {
        TC_LOG_ERROR("bg", "Isle of Conquest: Failed to spawn initial spirit guide!");
        return false;
    }
    ...
    if (!gunshipAlliance || !gunshipHorde)
    {
        TC_LOG_ERROR("bg", "Isle of Conquest: There was an error creating gunships!");
        return false;
    }
...
{
    if (!player || player->GetBGTeamId() >= 2)
        return;
...
uint32 BattlegroundIC::GetNextBanner(ICNodePoint* nodePoint, uint32 team, bool returnDefinitve)
{
    ...
    TC_LOG_ERROR("bg", "Isle Of Conquest: Unexpected return in GetNextBanner function");
    return 0;
}
...
void BattlegroundIC::HandleCapturedNodes(ICNodePoint* nodePoint, bool recapture)
{
    if (nodePoint->nodeType != NODE_TYPE_REFINERY && nodePoint->nodeType != NODE_TYPE_QUARRY)
    {
        if (!AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1+nodePoint->nodeType-2,
            ...
            TC_LOG_ERROR("bg", "Isle of Conquest: Failed to spawn spirit guide! point: %u, team: %u, ", nodePoint->nodeType, nodePoint->faction);
    }
    ...
    switch (nodePoint->gameobject_type)
    {
        case BG_IC_GO_HANGAR_BANNER:
            ...
            //TC_LOG_ERROR("bg", "BG_IC_GO_HANGAR_BANNER CAPTURED Faction: %u", nodePoint->faction);
...
void BattlegroundIC::DestroyGate(Player* player, GameObject* go)
{
    ...
    if (go->GetEntry() == GO_HORDE_GATE_1 || go->GetEntry() == GO_HORDE_GATE_2 || go->GetEntry() == GO_HORDE_GATE_3)
    {
        if (!GetBgMap()->GetCreature(BgCreatures[BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].type]) && !AddCreature(BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].entry, BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].type, BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].team,
            ...
            RESPAWN_ONE_DAY))
            TC_LOG_ERROR("bg", "Isle of Conquest: There was an error spawning creature %u", BG_IC_NpcSpawnlocs[BG_IC_NPC_OVERLORD_AGMAR].entry);
    }
    else if (go->GetEntry() == GO_ALLIANCE_GATE_1 || go->GetEntry() == GO_ALLIANCE_GATE_2 || go->GetEntry() == GO_ALLIANCE_GATE_3)
    {
        if (!GetBgMap()->GetCreature(BgCreatures[BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].type]) && !AddCreature(BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].entry, BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].type, BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].team,
            ...
            RESPAWN_ONE_DAY))
            TC_LOG_ERROR("bg", "Isle of Conquest: There was an error spawning creature %u", BG_IC_NpcSpawnlocs[BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE].entry);
    }
