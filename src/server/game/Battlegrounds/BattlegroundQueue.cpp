...
void BattlegroundQueue::RemovePlayer(uint64 guid, bool decreaseInvitedCount)
{
    ...
    if (decreaseInvitedCount && group->IsInvitedToBGInstanceGUID)
        if (Battleground* bg = sBattlegroundMgr->GetBattleground(group->IsInvitedToBGInstanceGUID, (group->BgTypeId == BATTLEGROUND_AA || group->BgTypeId == BATTLEGROUND_RATED_10_VS_10) ? BATTLEGROUND_TYPE_NONE : group->BgTypeId))
    ...
    else if (bg_template->isArena() || bg_template->IsRBG())
    {
        ...
        for (uint8 i = BG_QUEUE_PREMADE_ALLIANCE; i < BG_QUEUE_NORMAL_ALLIANCE; i++)
        {
            ...
            for (GroupsQueueType::iterator itr2 = m_QueuedGroups[bracket_id][i].begin(); itr2 != m_QueuedGroups[bracket_id][i].end(); ++itr2)
            {
                ...
                if (!(*itr2)->IsInvitedToBGInstanceGUID && ((*itr2)->MatchmakerRating == mmr))
                {
        ...
        if (found == 1)
        {
            for (GroupsQueueType::iterator itr3 = m_QueuedGroups[bracket_id][team].begin(); itr3 != m_QueuedGroups[bracket_id][team].end(); ++itr3)
            {
                if (!(*itr3)->IsInvitedToBGInstanceGUID
                    && (((*itr3)->MatchmakerRating >= MinRating && (*itr3)->MatchmakerRating <= MaxRating))
                    && (*itr_teams[0])->GroupId != (*itr3)->GroupId && abs(int32((*itr3)->MatchmakerRating - mmr)) < differenceOppMMR)
                {
                    itr_teams[1] = itr3;
            ...
            for (GroupsQueueType::iterator itr = m_QueuedGroups[BG_BRACKET_ID_LAST][j].begin(); itr != m_QueuedGroups[BG_BRACKET_ID_LAST][j].end(); ++itr)
            {
                if (GroupQueueInfo* _info = (*itr))
                {
                    ...
                    if (_info->RegTimer >= rUpdateTimer)
                    {
                        ...
                        GroupsQueueType::iterator itr_teams[BG_TEAMS_COUNT];
                        itr_teams[BG_TEAM_ALLIANCE] = itr;
                        uint32 maxRatingDifference = sBattlegroundMgr->GetMaxRatingDifference();
                        uint32 MinRating = (_info->MatchmakerRating <= maxRatingDifference) ? 0 : _info->MatchmakerRating - maxRatingDifference - _info->QueueLvl;
                        uint32 MaxRating = _info->MatchmakerRating + maxRatingDifference + _info->QueueLvl;
                        int32 differenceOppMMR = maxRatingDifference + _info->QueueLvl;
                        bool findOpp = false;

                        for (GroupsQueueType::iterator itr2 = m_QueuedGroups[BG_BRACKET_ID_LAST][j].begin(); itr2 != m_QueuedGroups[BG_BRACKET_ID_LAST][j].end(); ++itr2)
                            if (GroupQueueInfo* _info2 = (*itr2))
                                if (_info2->GroupId != _info->GroupId && !_info2->IsInvitedToBGInstanceGUID)
                                    if (_info2->MatchmakerRating >= MinRating && _info2->MatchmakerRating <= MaxRating)
                                        if (abs(int32(_info2->MatchmakerRating - _info->MatchmakerRating)) < differenceOppMMR)
                                        {
                                            differenceOppMMR = abs(int32(_info2->MatchmakerRating - _info->MatchmakerRating));
                                            itr_teams[BG_TEAM_HORDE] = itr2;
                                            findOpp = true;
                                        }
