...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 JoinType, bool isRated, bool isPremade, uint32 mmr)
{
    ...
    {
        //sWorld->SendWorldText(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_JOIN, "-", ginfo->JoinType, ginfo->JoinType, ginfo->ArenaTeamRating);
    }
...
void BattlegroundQueue::RemovePlayer(uint64 guid, bool decreaseInvitedCount)
{
    ...
    //        sWorld->SendWorldText(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_EXIT, Team->GetName().c_str(), group->JoinType, group->JoinType, group->ArenaTeamRating);
    ...
    else if (!group->IsInvitedToBGInstanceGUID && group->IsRated)
    {
        ...
        if (Player* plr2 = ObjectAccessor::FindPlayer(group->Players.begin()->first))
        {
            ...
            WorldPacket data;
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, plr2, queueSlot, STATUS_NONE, plr2->GetBattlegroundQueueJoinTime(group->BgTypeId), 0, group->JoinType);
