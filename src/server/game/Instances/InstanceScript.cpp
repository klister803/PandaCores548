...
void InstanceScript::HandleGameObject(uint64 GUID, bool open, GameObjectPtr go)
{
    if (!go)
...
void InstanceScript::UpdateMinionState(CreaturePtr minion, EncounterState state)
{
    switch (state)
    {
        case NOT_STARTED:
...
void InstanceScript::UpdateDoorState(GameObjectPtr door)
{
...
void InstanceScript::AddDoor(GameObjectPtr door, bool add)
{
    DoorInfoMap::iterator lower = doors.lower_bound(door->GetEntry());
    DoorInfoMap::iterator upper = doors.upper_bound(door->GetEntry());
    if (lower == upper)
...
void InstanceScript::AddMinion(CreaturePtr minion, bool add)
{
...
void InstanceScript::DoUseDoorOrButton(uint64 uiGuid, uint32 uiWithRestoreTime, bool bUseAlternativeState)
{
    ...
    GameObjectPtr go = instance->GetGameObject(uiGuid);
...
{
    if (GameObjectPtr go = instance->GetGameObject(uiGuid))
    {
...
void InstanceScript::DoUpdateAchievementCriteria(AchievementCriteriaTypes type, uint32 miscValue1 /*= 0*/, uint32 miscValue2 /*= 0*/, UnitPtr unit /*= nullptr*/)
{
...
bool InstanceScript::CheckAchievementCriteriaMeet(uint32 criteria_id, constPlayerPtr /*source*/, constUnitPtr /*target*/ /*= nullptr*/, uint32 /*miscvalue1*/ /*= 0*/)
{
...
void InstanceScript::SendEncounterUnit(uint32 type, UnitPtr unit /*= nullptr*/, uint8 param1 /*= 0*/, uint8 param2 /*= 0*/)
{
...
void InstanceScript::UpdateEncounterState(EncounterCreditType type, uint32 creditEntry, UnitPtr source)
{
