...
void RARunnable::run()
{
    ...
    {
        TC_LOG_ERROR("server", "Trinity RA can not bind to port %d on %s", raport, stringip.c_str());
        return;
    }

    TC_LOG_INFO("server", "Starting Trinity RA on port %d on %s", raport, stringip.c_str());
    ...
    TC_LOG_DEBUG("server", "Trinity RA thread exiting");
}
