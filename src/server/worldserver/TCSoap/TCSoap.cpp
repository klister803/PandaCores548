...
void TCSoapRunnable::run()
{
    ...
    {
        TC_LOG_ERROR("server", "TCSoap: couldn't bind to %s:%d", m_host.c_str(), m_port);
        exit(-1);
    }

    TC_LOG_INFO("server", "TCSoap: bound to http://%s:%d", m_host.c_str(), m_port);
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    if (acc)
    {
        TC_LOG_ERROR("server", "TCSoap: Error while acquiring lock, acc = %i, errno = %u", acc, errno);
    }
