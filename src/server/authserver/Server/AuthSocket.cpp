...
{
    TC_LOG_DEBUG("server", "'%s:%d' Accepting connection", socket().getRemoteAddress().c_str(), socket().getRemotePort());
}

void AuthSocket::OnClose(void)
{
    TC_LOG_DEBUG("server", "AuthSocket::OnClose");
}
...
void AuthSocket::OnRead()
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < AUTH_TOTAL_COMMANDS; ++i)
        {
            ...
            {
                TC_LOG_DEBUG("server", "Got data for cmd %u recv length %u", (uint32)_cmd, (uint32)socket().recv_len());

                if (!(*this.*table[i].handler)())
                {
                    TC_LOG_DEBUG("server", "Command handler failed for cmd %u recv length %u", (uint32)_cmd, (uint32)socket().recv_len());
                    return;
                }
                break;
            }
        ...
        {
            TC_LOG_ERROR("server", "Got unknown packet from '%s'", socket().getRemoteAddress().c_str());
            socket().shutdown();
            return;
        }
    }
...
{
    TC_LOG_DEBUG("server", "Entering _HandleLogonChallenge");
    ...
    TC_LOG_DEBUG("server", "[AuthChallenge] got header, body is %#04x bytes", remaining);
    ...
    TC_LOG_DEBUG("server", "[AuthChallenge] got full packet, %#04x bytes", ch->size);
    TC_LOG_DEBUG("server", "[AuthChallenge] name(%d): '%s'", ch->I_len, ch->I);
    ...
    if (result)
    {
        ...
        TC_LOG_DEBUG("server", "'%s:%d' [AuthChallenge] Banned ip tries to login!",socket().getRemoteAddress().c_str(), socket().getRemotePort());
    }
    else
    {
        ...
        if (res2)
        {
            ...
            {
                TC_LOG_DEBUG("server", "[AuthChallenge] Account '%s' is locked to IP - '%s'", _login.c_str(), fields[3].GetCString());
                TC_LOG_DEBUG("server", "[AuthChallenge] Player address is '%s'", ip_address.c_str());

                if (strcmp(fields[3].GetCString(), ip_address.c_str()))
                {
                    TC_LOG_DEBUG("server", "[AuthChallenge] Account IP differs");
                    pkt << uint8(WOW_FAIL_SUSPENDED);
                    locked = true;
                }
                else
                    TC_LOG_DEBUG("server", "[AuthChallenge] Account IP matches");
            }
            else
                TC_LOG_DEBUG("server", "[AuthChallenge] Account '%s' is not locked to ip", _login.c_str());
            ...
            if (!locked)
            {
                ...
                if (banresult)
                {
                    ...
                    {
                        pkt << (uint8)WOW_FAIL_BANNED;
                        TC_LOG_DEBUG("server", "'%s:%d' [AuthChallenge] Banned account %s tried to login!", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str ());
                    }
                    else
                    {
                        pkt << (uint8)WOW_FAIL_SUSPENDED;
                        TC_LOG_DEBUG("server", "'%s:%d' [AuthChallenge] Temporarily banned account %s tried to login!", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str ());
                    }
                }
                else
                {
                    ...
                    TC_LOG_DEBUG("server", "'%s:%d' [AuthChallenge] account %s is using '%c%c%c%c' locale (%u)", socket().getRemoteAddress().c_str(), socket().getRemotePort(),
...
{
    TC_LOG_DEBUG("server", "Entering _HandleLogonProof");
    // Read the packet
    ...
    {
        TC_LOG_DEBUG("server", "'%s:%d' User '%s' successfully authenticated", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
    ...
    else
    {
        ...
        TC_LOG_DEBUG("server", "'%s:%d' [AuthChallenge] account %s tried to login with invalid password!", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str ());
            ...
            if (PreparedQueryResult loginfail = LoginDatabase.Query(stmt))
            {
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    if (WrongPassBanType)
                    {
                        ...
                        TC_LOG_DEBUG("server", "'%s:%d' [AuthChallenge] account %s got banned for '%u' seconds because it failed to authenticate '%u' times",
                    ...
                    else
                    {
                        ...
                        TC_LOG_DEBUG("server", "'%s:%d' [AuthChallenge] IP %s got banned for '%u' seconds because account %s failed to authenticate '%u' times",
...
{
    TC_LOG_DEBUG("server", "Entering _HandleReconnectChallenge");
    if (socket().recv_len() < sizeof(sAuthLogonChallenge_C))
        return false;
    ...
    uint16 remaining = ((sAuthLogonChallenge_C *)&buf[0])->size;
    TC_LOG_DEBUG("server", "[ReconnectChallenge] got header, body is %#04x bytes", remaining);
    ...
    // Read the remaining of the packet
    socket().recv((char *)&buf[4], remaining);
    TC_LOG_DEBUG("server", "[ReconnectChallenge] got full packet, %#04x bytes", ch->size);
    TC_LOG_DEBUG("server", "[ReconnectChallenge] name(%d): '%s'", ch->I_len, ch->I);
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", "'%s:%d' [ERROR] user %s tried to login and we cannot find his session key in the database.", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
        socket().shutdown();
        return false;
    }
...
{
    TC_LOG_DEBUG("server", "Entering _HandleReconnectProof");
    // Read the packet
    ...
    {
        TC_LOG_ERROR("server", "'%s:%d' [ERROR] user %s tried to login, but session is invalid.", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
        socket().shutdown();
        return false;
    }
}
...
{
    TC_LOG_DEBUG("server", "Entering _HandleRealmList");
    ...
    if (!result)
    {
        TC_LOG_ERROR("server", "'%s:%d' [ERROR] user %s tried to login but we cannot find him in the database.", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
        socket().shutdown();
        return false;
    }
...
{
    TC_LOG_DEBUG("server", "Entering _HandleXferResume");
    // Check packet length and patch existence
    if (socket().recv_len() < 9 || !pPatch)
    {
        TC_LOG_ERROR("server", "Error while resuming patch transfer (wrong packet)");
        return false;
    }
...
{
    TC_LOG_DEBUG("server", "Entering _HandleXferCancel");
...
{
    TC_LOG_DEBUG("server", "Entering _HandleXferAccept");

    // Check packet length and patch existence
    if (!pPatch)
    {
        TC_LOG_ERROR("server", "Error while accepting patch transfer (wrong packet)");
        return false;
    }
...
void Patcher::LoadPatchMD5(char *szFileName)
{
    ...
    if (!pPatch)
    {
        TC_LOG_ERROR("server", "Error loading patch %s\n", path.c_str());
        return;
    }
