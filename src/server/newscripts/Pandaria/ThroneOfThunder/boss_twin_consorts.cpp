    ...
    public:
        ...
        struct boss_twin_consortsAI : public generic_boss_twin_consortsAI
        {
            ...
            void EnterCombat(Unit* who)
            {
                ...
                {
                case NPC_LULIN:
                    events.ScheduleEvent(EVENT_INTRO_DAY,   180000);
                    events.ScheduleEvent(EVENT_NIGHTMARE,    50000);
                    break;
                case NPC_SULIN:
                    events.ScheduleEvent(EVENT_TEARS_OF_SUN, 10000);
                    break;
            ...
            void DoAction(int32 const action)
            {
                ...
                case ACTION_INTRO_DAY:
                    ...
                    events.ScheduleEvent(EVENT_FAN_OF_FLAME, 30000);
                    events.ScheduleEvent(EVENT_LIGHT_OF_DAY,  8000);
                    break;
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_TEARS_OF_SUN:
                        ...
                        events.ScheduleEvent(EVENT_TEARS_OF_SUN_END, 1000);
                        break;
                    case EVENT_TEARS_OF_SUN_END:
                        me->SetVisible(false);
                        events.ScheduleEvent(EVENT_TEARS_OF_SUN, 40000);
                        break;
                    case EVENT_FAN_OF_FLAME:
                        if (me->getVictim())
                            DoCast(me->getVictim(), SPELL_FAN_OF_FLAME);
                        events.ScheduleEvent(EVENT_FAN_OF_FLAME, 12000);
                        break;
