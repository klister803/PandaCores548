        ...
        void OnSpellClick(Unit* clicker)
        {
            if (Player* p = clicker->ToPlayer())
            {
                if (p->GetPower(POWER_ALTERNATE_POWER))
                {
                    ...
                    default:
                        p->CastSpell(p, SPELL_TEST_OF_SERENITY, false);
                        TC_LOG_ERROR("player", "Script::npc_norushen_purifying_light: Player %s has not localized role specID.", p->ToString().c_str(), role);
                        break;
                    }
                }
            }
    ...
    class spell_norushen_challenge_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            { 
                ...
                {
                    if (Creature* norush = GetTarget()->GetCreature(*GetTarget(), instance->GetData64(NPC_NORUSHEN)))
                        norush->AI()->ZoneTalk(TEXT_GENERIC_10, GetTarget()->GetGUID());
                    GetTarget()->CastSpell(GetTarget(), getPhaseSpell(), true);
                    GetTarget()->SetPhaseId(GetTarget()->GetGUID(), true);
                    switch (GetId())
                    {
                    case SPELL_TEST_OF_SERENITY:    //dd
                    {
