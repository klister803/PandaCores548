...
public:
    ...
    struct boss_selin_fireheartAI : public ScriptedAI
    {
        boss_selin_fireheartAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            if (instance)
            {
                ...
                for (uint8 i = 0; i < size; ++i)
                {
                    ...
                    TC_LOG_DEBUG("server", "Selin: Adding Fel Crystal " UI64FMTD " to list", guid);
        ...
        void Reset()
        {
            ...
            } else TC_LOG_ERROR("server", ERROR_INST_DATA);
        ...
        void MovementInform(uint32 type, uint32 id)
        {
            if (type == POINT_MOTION_TYPE && id == 1)
            {
                ...
                else
                {
                    ...
                    TC_LOG_ERROR("server", "Selin Fireheart unable to drain crystal as the crystal is either dead or despawned");
                    DrainingCrystal = false;
                }
            }
        }
