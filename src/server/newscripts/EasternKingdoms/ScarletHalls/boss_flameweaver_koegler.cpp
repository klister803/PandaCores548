...
public:
    ...
    struct boss_flameweaver_koeglerAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/) 
        {
            ...
            events.ScheduleEvent(EVENT_PYROBLAST, 4000);
            events.ScheduleEvent(EVENT_QUICKENED_MIND, 8000);
            events.ScheduleEvent(EVENT_FIREBALL_VOLLEY, 14000);
            events.ScheduleEvent(EVENT_TELEPORT, 30000);
            events.ScheduleEvent(EVENT_BOOK_BURNER, 22000);
            _EnterCombat();
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_PYROBLAST:
                        ...
                        events.ScheduleEvent(EVENT_PYROBLAST, 1000, 0);
                        break;
                    case EVENT_QUICKENED_MIND:
                        DoCast(SPELL_QUICKENED_MIND);
                        events.ScheduleEvent(EVENT_QUICKENED_MIND, 30000);
                        break;
                    case EVENT_FIREBALL_VOLLEY:
                        DoCast(SPELL_FIREBALL_VOLLEY);
                        events.ScheduleEvent(EVENT_FIREBALL_VOLLEY, 30000);
                        break;
                    case EVENT_TELEPORT:
                        ...
                        Talk(SAY_BREATH);
                        events.ScheduleEvent(EVENT_TELEPORT, 46000);
                        events.ScheduleEvent(EVENT_BREATH_START, 1000);
                        break;
                    case EVENT_BREATH_START:
                        Talk(SAY_BREATH_WARNING);
                        Orient = true;
                        DoCast(SPELL_DRAGON_BREATH);
                        events.ScheduleEvent(EVENT_BREATH_FINISH, 1000);
                        break;
                    ...
                    case EVENT_BOOK_BURNER:
                        ...
                        events.ScheduleEvent(EVENT_BOOK_BURNER, 32000);
                        break;
                }
            }
