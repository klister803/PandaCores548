...
public:
    ...
    struct boss_scar_shellAI : public BossAI
    {
        ...
        {
            _EnterCombat();
            events.ScheduleEvent(EVENT_STONE_SPIN, 16000);
            events.ScheduleEvent(EVENT_CRUSHING_BITE, 9000);
        }
        ...
        void MovementInform(uint32 type, uint32 id)
        {
            if (type == POINT_MOTION_TYPE && id != 1)
                events.ScheduleEvent(EVENT_STONE_SPIN2, 0);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CRUSHING_BITE:
                        ...
                        events.ScheduleEvent(EVENT_CRUSHING_BITE, 12000);
                        break;
                }
            }
