...
public:
    ...
    struct npc_scenario_gurgthockAI : public ScriptedAI
    {
...
public:
    ...
    struct boss_satay_byuAI : public BossAI
    {
        ...
        {
            _EnterCombat();
            events.ScheduleEvent(EVENT_SPEED_JINJA, 8000);
...
public:
    ...
    struct boss_cloudbender_koboAI : public BossAI
    {
        ...
        {
            _EnterCombat();
            events.ScheduleEvent(EVENT_NIMBUS, 20000);
            events.ScheduleEvent(EVENT_FLYING_KICK, 10000);
            events.ScheduleEvent(EVENT_CYCLONE_KICK, 16000);
        }
        ...
        void MovementInform(uint32 type, uint32 id)
        {
            ...
            switch (id)
            {
                ...
                case 2:
                    DoCast(SPELL_NIMBUS);
                    events.ScheduleEvent(EVENT_POINT_1, 0);
                    break;
                case 3:
                    events.ScheduleEvent(EVENT_POINT_2, 0);
                    break;
                case 4:
                    events.ScheduleEvent(EVENT_POINT_3, 0);
                    break;
                case 5:
                    events.ScheduleEvent(EVENT_POINT_4, 0);
                    break;
                case 6:
                    events.ScheduleEvent(EVENT_POINT_5, 0);
                    break;
                case 7:
                    me->SetCanFly(false);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CYCLONE_KICK:
                        ...
                        events.ScheduleEvent(EVENT_CYCLONE_KICK, 22000);
                        break;
                }
            }
            DoMeleeAttackIfReady();
...
public:
    ...
    struct boss_maki_waterbladeAI : public BossAI
    {
        ...
        {
            _EnterCombat();
            events.ScheduleEvent(EVENT_CRASHING_SLASH, 4000);
            events.ScheduleEvent(EVENT_WHIRLPOOL, 54000);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CRASHING_SLASH:
                        ...
                        events.ScheduleEvent(EVENT_CRASHING_SLASH, 8000);
                        break;
