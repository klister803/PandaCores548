                ...
                if (indexTable.asT)
                {
                    ...
                    do
                    {
                        ...
                        if (sql->indexPos >= 0)
                        {
                            ...
                            {
                                TC_LOG_ERROR("server", "Index %d already exists in dbc '%s', overwriting", id, sql->sqlTableName.c_str());
                        ...
                        for (; columnNumber < sql->formatString->size(); ++columnNumber)
                        {
                            ...
                            else if ((*sql->formatString)[columnNumber] == FT_SQL_PRESENT)
                            {
                                ...
                                {
                                    TC_LOG_ERROR("server", "SQL and DBC format strings are not matching for table: '%s'", sql->sqlTableName.c_str());
                                    return false;
                                }
