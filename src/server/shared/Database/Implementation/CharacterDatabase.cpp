...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(CHAR_SEL_PET_SLOTS_DETAIL, "SELECT owner, slot, id, entry, level, name, modelid FROM character_pet WHERE owner = ? AND slot >= ? AND slot <= ? ORDER BY slot", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_UDP_CHAR_PET_SLOT_BY_ID, "UPDATE character_pet SET slot = ? WHERE id = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHAR_DATA_FOR_GUILD, "SELECT name, level, class, zone, account, re.standing FROM characters c LEFT JOIN character_reputation re ON re.guid = c.guid AND re.faction = 1168 WHERE c.guid = ?", CONNECTION_SYNCH)
    ...
    // Currency
    PREPARE_STATEMENT(CHAR_SEL_PLAYER_CURRENCY, "SELECT currency, week_count, total_count, season_total, flags FROM character_currency WHERE guid = ?", CONNECTION_ASYNC);
