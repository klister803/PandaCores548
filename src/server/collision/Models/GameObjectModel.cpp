...
void LoadGameObjectModelList(std::string const& dataPath)
{
    ...
    if (!model_list_file)
    {
        VMAP_ERROR_LOG("maps", "Unable to open '%s' file.", VMAP::GAMEOBJECT_MODELS);
        return;
    }
    ...
    while (true)
    {
        ...
        {
            VMAP_ERROR_LOG("maps", "File '%s' seems to be corrupted!", VMAP::GAMEOBJECT_MODELS);
            break;
        }

        if (v1.isNaN() || v2.isNaN())
        {
            VMAP_ERROR_LOG("maps", "File '%s' Model '%s' has invalid v1%s v2%s values!", VMAP::GAMEOBJECT_MODELS, std::string(buff, name_length).c_str(), v1.toString().c_str(), v2.toString().c_str());
            continue;
        }

        // VMAP_ERROR_LOG("maps", "File '%s' Model '%s' displayId %u", VMAP::GAMEOBJECT_MODELS, std::string(buff, name_length).c_str(), displayId);
...
bool GameObjectModel::initialize(std::unique_ptr<GameObjectModelOwnerBase> modelOwner, std::string const& dataPath)
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        G3D::Vector3 pos(iBound.corner(i));
...
bool GameObjectModel::intersectLine(G3D::Ray const& ray, float& maxDist, bool stopAtFirstHit, uint32 ph_mask) const
{
    if (!(phasemask & ph_mask) || !owner->IsSpawned())
        return false;
    ...
    // TC_LOG_ERROR("maps", "GameObjectModel::intersectLine distance %f hit %u iInvScale %f iScale %f", distance, hit, iInvScale, iScale);
