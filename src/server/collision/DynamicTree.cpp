...
struct DynamicTreeIntersectionCallback
{
    ...
    uint32 phase_mask;
    GameObject* _go;
    DynamicTreeIntersectionCallback(uint32 phasemask) : did_hit(false), phase_mask(phasemask), _go(0) { }
...
{
    DynamicTreeisInLineOfSightCallback(uint32 phasemask) : _didHit(false), phase_mask(phasemask), _go(0) { }

    bool operator()(G3D::Ray const& r, GameObjectModel const& obj, float& distance)
    {
        _didHit = obj.intersectLine(r, distance, true, phase_mask);
        if (_didHit)
            _go = const_cast<GameObject*>(obj.owner->GetOwner());
        return _didHit;
    }
...
bool DynamicMapTree::getIntersectionTime(const uint32 phasemask, const G3D::Ray& ray,
                                         const G3D::Vector3& endPos, float& maxDist, DynamicTreeCallback* dCallback) const
{
...
bool DynamicMapTree::getObjectHitPos(const uint32 phasemask, const G3D::Vector3& startPos,
                                     ...
                                     float modifyDist, DynamicTreeCallback* dCallback) const
{
    bool result = false;
    ...
    float dist = maxDist;
    if (getIntersectionTime(phasemask, ray, endPos, dist, dCallback))
    {
...
bool DynamicMapTree::isInLineOfSight(float x1, float y1, float z1, float x2, float y2, float z2, uint32 phasemask, DynamicTreeCallback* dCallback) const
{
    ...
    DynamicTreeisInLineOfSightCallback callback(phasemask);
