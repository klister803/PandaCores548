    ...
    public:
        ...
        struct instance_dragon_soul_InstanceMapScript : public InstanceScript, public instance_dragon_soul_trash_accessor
        {
            ...
            void OnPlayerEnter(Player* pPlayer)
            {
                ...
                if (GetBossState(DATA_MADNESS) == DONE && pPlayer->GetAreaId() == 5893)
            ...
            void OnCreatureCreate(Creature* pCreature)
            {
                ...
                switch (pCreature->GetEntry())
                {
                    ...
                    case NPC_TWILIGHT_ASSAULTER_STALKER:
                    {
                        ...
                        sLog->outError(LOG_FILTER_TSCR, "instance_dragon_soul: NPC_TWILIGHT_ASSAULTER_STALKER of unknown DB GUID was spawned: %u", pCreature->GetDBTableGUIDLow());
                        break;
                    }
                    case NPC_EIENDORMI:
                    case NPC_VALEERA:
            ...
            void OnUnitDeath(Unit* unit)
            {
                if (Player* player = unit->ToPlayer())
                {
                    ...
                    {
                        player->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 94644, 0, player, player);
                        player->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET2, 94655, 0, player, player);
                    }
                }
            }
            ...
            bool CheckAchievementCriteriaMeet(uint32 criteria_id, Player const* /*source*/, Unit const* /*target*/, uint32 /*miscvalue1*/)
            {
                switch (criteria_id)
                {
                    ...
                    case CRITERIA_DESTROYERS_END:
                        return !IsLFR();
                    default:
                        sLog->outError(LOG_FILTER_GENERAL, "Achievement system call InstanceScript::CheckAchievementCriteriaMeet but instance script for map %u not have implementation for achievement criteria %u", instance->GetId(), criteria_id);
                        break;
                }
