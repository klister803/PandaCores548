...
void hyjalAI::SummonNextWave(const Wave wave[18], uint32 Count, float Base[4][3])
{
    ...
    if (!instance)
    {
        TC_LOG_ERROR("server", ERROR_INST_DATA);
        return;
    }
    ...
    if (!wave[Count].IsBoss)
    {
        ...
        {
            NextWaveTimer = 15000;
            TC_LOG_DEBUG("server", "HyjalAI: debug mode is enabled. Next Wave in 15 seconds");
        }
    }
    else
    {
...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    cell.Visit(pair, Trinity::makeGridVisitor(creature_searcher), *(me->GetMap()), *me, me->CalcVisibilityRange());
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    cell.Visit(p, Trinity::makeGridVisitor(worker), *me->GetMap(), *me, me->CalcVisibilityRange());
}
