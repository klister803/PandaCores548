...
public:
    ...
    {
        boss_mal_ganisAI(CreaturePtr creature) : ScriptedAI(creature)
        {
        ...
        void UpdateAI(const uint32 diff)
        {
            switch (Phase)
            {
                case COMBAT:
                    ...
                    if (CreaturePtr pArthas = me->GetCreature(TO_WORLDOBJECT(me), instance ? instance->GetData64(DATA_ARTHAS) : 0))
                        if (pArthas->isDead())
                        {
                            EnterEvadeMode();
                            me->DisappearAndDie();
