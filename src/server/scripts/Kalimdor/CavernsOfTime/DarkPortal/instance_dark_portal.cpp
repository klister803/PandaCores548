...
public:
    ...
    struct instance_dark_portal_InstanceMapScript : public InstanceScript
    {
        ...
        void SetData(uint32 type, uint32 data)
        {
            ...
            case TYPE_MEDIVH:
                ...
                else
                {
                    if (data == IN_PROGRESS)
                    {
                        TC_LOG_DEBUG("server", "Instance Dark Portal: Starting event.");
                        InitWorldState();
                    ...
                    if (data == DONE)
                    {
                        ...
                        TC_LOG_DEBUG("server", "Instance Dark Portal: Event completed.");
        ...
        Creature* SummonedPortalBoss(Creature* me)
        {
            ...
            TC_LOG_DEBUG("server", "Instance Dark Portal: Summoning rift boss entry %u.", entry);
            ...
            TC_LOG_DEBUG("server", "Instance Dark Portal: What just happened there? No boss, no loot, no fun...");
            return NULL;
        }
        ...
        void DoSpawnPortal()
        {
            if (Creature* pMedivh = instance->GetCreature(MedivhGUID))
            {
                ...
                TC_LOG_DEBUG("server", "Instance Dark Portal: Creating Time Rift at locationId %i (old locationId was %u).", tmp, CurrentRiftId);
