...
public:
    ...
    bool OnUse(PlayerPtr player, ItemPtr item, SpellCastTargets const& /*targets*/)
    {
...
public:
    ...
    bool OnUse(PlayerPtr player, ItemPtr /*ItemPtr/, SpellCastTargets const& /*targets*/)
    {
        ...
        {
            if (CreaturePtr nether = player->SummonCreature(22408, player->GetPositionX(), player->GetPositionY()+20, player->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 180000))
...
public:
    ...
    bool OnExpire(PlayerPtr player, ItemTemplate const* /*pItemProto*/)
    {
...
public:
    ...
    bool OnUse(PlayerPtr player, ItemPtr /*ItemPtr/, SpellCastTargets const & /*targets*/)
    {
        ...
        if (TempSummonPtr summon = player->SummonCreature(NPC_NESINGWARY_TRAPPER, x, y, z, go->GetOrientation(), TEMPSUMMON_DEAD_DESPAWN, 1000))
        {
...
public:
    ...
    bool OnUse(PlayerPtr player, ItemPtr /*ItemPtr/, const SpellCastTargets & /*targets*/)
    {
...
public:
    ...
    bool OnUse(PlayerPtr player, ItemPtr /*ItemPtr/, const SpellCastTargets & /*targets*/)
    {
        ...
        CreaturePtr pMammoth = player->FindNearestCreature(NPC_TRAPPED_MAMMOTH_CALF, 5.0f);
        if (!pMammoth)
            return false;
...
public:
    ...
    bool OnUse(PlayerPtr player, ItemPtr item, const SpellCastTargets & /*targets*/)
    {
        if (player->GetQuestStatus(QUEST_THE_EMISSARY) == QUEST_STATUS_INCOMPLETE)
        {
            if (CreaturePtr pLeviroth = player->FindNearestCreature(NPC_LEVIROTH, 10.0f)) // spell range
            {
