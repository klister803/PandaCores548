...
class npc_blade_rush : public CreatureScript
{
    public:
        npc_blade_rush() : CreatureScript("npc_blade_rush") {}

        struct npc_blade_rushAI : public ScriptedAI
        {
            npc_blade_rushAI(Creature* creature) : ScriptedAI(creature)
            {
                instance = creature->GetInstanceScript();
                me->SetReactState(REACT_PASSIVE);
            ...
            void Reset()
            {
                me->AddAura(SPELL_BLADE_RUSH_DUMMY, me);
            }
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new npc_blade_rushAI(creature);
        }
};
