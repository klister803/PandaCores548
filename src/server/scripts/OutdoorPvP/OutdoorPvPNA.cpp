...
void OPvPCapturePointNA::HandlePlayerLeave(Player* player)
{
    if (player && player->IsInWorld())
        player->m_Functions.AddFunction([player]() -> void { if (!player) return; player->SendUpdateWorldState(NA_UI_TOWER_SLIDER_DISPLAY, 0); }, player->m_Functions.CalculateTime(100));
