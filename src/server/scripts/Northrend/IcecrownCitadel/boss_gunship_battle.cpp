    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* pCreature)
        {
            ...
            if (pInstance && pInstance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE && pInstance->GetData(DATA_GUNSHIP_EVENT) != DONE)
            {
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Skip Gunship Battle", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+3);
        ...
        bool OnGossipSelect(Player* player, Creature* pCreature, uint32 /*sender*/, uint32 action)
        {
            ...
            if (action == GOSSIP_ACTION_INFO_DEF+3)
            {
                ...
                pCreature->MonsterSay("Saurfang teleporter unlocked!", LANG_UNIVERSAL, player->GetGUID());
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_OUTRO_ALLIANCE_2:
                            StopFlyShip(skybreaker);
                            me->SummonGameObject(RAID_MODE(GO_CAPITAN_CHEST_A_10N, GO_CAPITAN_CHEST_A_25N, GO_CAPITAN_CHEST_A_10H, GO_CAPITAN_CHEST_A_25H), -576.077637f, 2185.497314f, 538.884521f, 0, 0, 0, 0, 0, 100000);
                            _instance->SetBossState(DATA_GUNSHIP_EVENT, DONE);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_OUTRO_HORDE_2:
                            StopFlyShip(orgrimmar);
                            me->SummonGameObject(RAID_MODE(GO_CAPITAN_CHEST_H_10N, GO_CAPITAN_CHEST_H_25N, GO_CAPITAN_CHEST_H_10H, GO_CAPITAN_CHEST_H_25H), -576.077637f, 2185.497314f, 538.884521f, 0, 0, 0, 0, 0, 100000);
                            _instance->SetBossState(DATA_GUNSHIP_EVENT, DONE);
                            me->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
                            me->GetMotionMaster()->MovePoint(0, -590.700f, 2213.01f, 539.1f);
                            break;
...
class spell_gb_canon_gun_org : public SpellScriptLoader
{
    public:
        spell_gb_canon_gun_org() : SpellScriptLoader("spell_gb_canon_gun_org") { }

        class spell_gb_canon_gun_org_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gb_canon_gun_org_SpellScript);

            void FilterTargets(std::list<WorldObject*>& unitList)
            {
                for (std::list<WorldObject*>::iterator itr = unitList.begin(); itr != unitList.end();)
                {
                    if ((*itr)->GetEntry() == NPC_GB_ORGRIMS_HAMMER)
                        ++itr;
                    else
                        unitList.erase(itr++);
                }
            ...
            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gb_canon_gun_org_SpellScript::FilterTargets, EFFECT_1, TARGET_UNIT_DEST_AREA_ENTRY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_gb_canon_gun_org_SpellScript();
        }
};

class spell_gb_canon_gun_sky : public SpellScriptLoader
{
    public:
        spell_gb_canon_gun_sky() : SpellScriptLoader("spell_gb_canon_gun_sky") { }

        class spell_gb_canon_gun_sky_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gb_canon_gun_sky_SpellScript);

            void FilterTargets(std::list<WorldObject*>& unitList)
            {
                for (std::list<WorldObject*>::iterator itr = unitList.begin(); itr != unitList.end();)
                {
                    if ((*itr)->GetEntry() == NPC_GB_SKYBREAKER)
                        ++itr;
                    else
                        unitList.erase(itr++);
                }
            ...
            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gb_canon_gun_sky_SpellScript::FilterTargets, EFFECT_1, TARGET_UNIT_DEST_AREA_ENTRY);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_gb_canon_gun_sky_SpellScript();
        }
};
...
void AddSC_boss_gunship_battle()
{
    ...
    new spell_gb_canon_gun_org();
    new spell_gb_canon_gun_sky();
}
