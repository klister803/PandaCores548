    ...
    void SummonedCreatureDespawn(Creature* pSummoned)
    {
        switch (pSummoned->GetEntry())
        {
            case NPC_LIGHT_ESSENCE:
                ...
                m_pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_POWERING_UP);
                break;
    ...
    void SpellHit(Unit* caster, SpellInfo const* spell)
    {
        if (caster->ToCreature() == me)
            if (spell->Effects[0].Effect == 136) //Effect Heal
                if (m_pInstance)
                    m_pInstance->SetData(DATA_HEALTH_TWIN_SHARED, me->GetHealth() + me->CountPctFromMaxHealth(spell->Effects[0].BasePoints));
    }
...
public:
    ...
    struct mob_unleashed_darkAI : public mob_unleashed_ballAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            {
                if (Player *pTarget = me->SelectNearestPlayer(3.0f))
                {
                    if (pTarget->isAlive())
                    {
...
class spell_twin_valkyr_shield : public SpellScriptLoader
{
    public:
        spell_twin_valkyr_shield() : SpellScriptLoader("spell_twin_valkyr_shield") { }

        class sspell_twin_valkyr_shield_AuraScript : public AuraScript
        {
            PrepareAuraScript(sspell_twin_valkyr_shield_AuraScript);

            void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_ENEMY_SPELL)
                {
                    GetTarget()->RemoveAurasDueToSpell(65876);
                    GetTarget()->RemoveAurasDueToSpell(65875);
                }
            }

            void Register()
            {
                OnEffectRemove += AuraEffectRemoveFn(sspell_twin_valkyr_shield_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB, AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const
        {
            return new sspell_twin_valkyr_shield_AuraScript();
        }
};
