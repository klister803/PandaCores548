...
public:
    ...
    struct instance_trial_of_the_champion_InstanceMapScript : public InstanceScript
    {
        ...
        void OnCreatureCreate(Creature* pCreature)
        {
            ...
            switch(pCreature->GetEntry())
            {
                ...
                case NPC_ARELAS:
                    if (TeamInInstance == HORDE)
                        pCreature->UpdateEntry(NPC_JAEREN, ALLIANCE);
            ...
            case DATA_KNIGHT:
                ...
                else if (uiData == DONE)
                {
                    if (Creature* pBlackKnight =  instance->GetCreature(uiBlackKnightGUID))
                    {
                        if (Creature* pAnnouncer = instance->GetCreature(uiAnnouncerGUID))
                        {
                            if (Creature* pGhoul = pBlackKnight->SummonCreature(NPC_RISEN_JAEREN,pAnnouncer->GetPositionX(),pAnnouncer->GetPositionY(),pAnnouncer->GetPositionZ(),pAnnouncer->GetOrientation()))
                            {
    ...
    uint64 GetData64(uint32 uiData)
    {
        switch(uiData)
        {
            ...
            case DATA_PALETRESS: return uiArgentChampionGUID;
