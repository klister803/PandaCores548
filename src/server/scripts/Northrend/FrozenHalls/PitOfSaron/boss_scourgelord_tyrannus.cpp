        ...
        struct boss_tyrannusAI : public BossAI
        {
            ...
            case 4:
                    if (Creature* rimeflang = GetClosestCreatureWithEntry(me, 36661, 325.0f))
                    {
                        ...
                        rimeflang->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        rimeflang->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
...
class boss_rimefang : public CreatureScript
{
    public:
        ...
        {
             boss_rimefangAI(Creature* creature) : BossAI(creature, DATA_RIMEFLANG),_vehicle(creature->GetVehicleKit())
            {
...
class player_overlord_brandAI : public PlayerAI
{
    public:
        ...
        {
            if (tyrannus && me && tyrannus->getVictim() && damage)
...
public:
    ...
    {
        Creature *pElandra = me->SummonCreature(NPC_ELANDRA, SummonLoc[0].x, SummonLoc[0].y, SummonLoc[0].z, SummonLoc[20].o, TEMPSUMMON_DEAD_DESPAWN, 0);
        if (pElandra)
            pElandra->GetMotionMaster()->MovePoint(0, MoveLoc2[3].x, MoveLoc2[3].y, MoveLoc2[3].z);
        Creature *pKoreln = me->SummonCreature(NPC_KORELN, SummonLoc[1].x, SummonLoc[1].y, SummonLoc[1].z, SummonLoc[21].o, TEMPSUMMON_DEAD_DESPAWN, 0);
        if (pKoreln)
    ...
    {
        Creature *pLoralen = me->SummonCreature(NPC_LORALEN, SummonLoc[0].x, SummonLoc[0].y, SummonLoc[0].z, SummonLoc[20].o, TEMPSUMMON_DEAD_DESPAWN, 0);
        if (pLoralen)
            pLoralen->GetMotionMaster()->MovePoint(0, MoveLoc2[3].x, MoveLoc2[3].y, MoveLoc2[3].z);
        Creature *pKelira = me->SummonCreature(NPC_KALIRA, SummonLoc[1].x, SummonLoc[1].y, SummonLoc[1].z, SummonLoc[21].o, TEMPSUMMON_DEAD_DESPAWN, 0);
        if (pKelira)
        ...
        if (pInstance->GetBossState(DATA_TYRANNUS) == DONE)
        {
            if(m_uiSpeech_Timer < uiDiff)
            {
                ...
                for(uint8 i = 0; i < 31; i++)
                    ...
                    for(uint8 i = 0; i < 31; i++)
                    {
                      if(Creature* pDefenderNPC = GetClosestCreatureWithEntry(me, NPC_SLAVE_HORDE_1, 1525.0f))
                        {
                        ...
                        if(Creature* pDefenderNPC = GetClosestCreatureWithEntry(me, NPC_SLAVE_HORDE_2, 1525.0f))
                        {
                        pDefenderNPC->CastSpell(pDefenderNPC, SPELL_FROST_BOMB, false);
                        me->Kill(pDefenderNPC);
                        pDefenderNPC->Kill(pDefenderNPC);
                        ...
                        if(Creature* pDefenderNPC = GetClosestCreatureWithEntry(me, NPC_SLAVE_HORDE_3, 1525.0f))
                        {
                        pDefenderNPC->CastSpell(pDefenderNPC, SPELL_FROST_BOMB, false);
                        pDefenderNPC->Kill(pDefenderNPC);
                        me->Kill(pDefenderNPC);
                    ...
                    for(uint8 i = 0; i < 31; i++)
                    {
                    if(Creature* pDefenderNPC = GetClosestCreatureWithEntry(me, NPC_SLAVE_ALY_1, 1525.0f))
                        {
                        me->Kill(pDefenderNPC);
                        pDefenderNPC->Kill(pDefenderNPC);
                        }
                        
                        if(Creature* pDefenderNPC = GetClosestCreatureWithEntry(me, NPC_SLAVE_ALY_2, 1525.0f))
                        {
                        me->Kill(pDefenderNPC);
                        pDefenderNPC->Kill(pDefenderNPC);
                        }
                        if(Creature* pDefenderNPC = GetClosestCreatureWithEntry(me, NPC_SLAVE_ALY_3, 1525.0f))
                        {
                        me->Kill(pDefenderNPC);
                        pDefenderNPC->Kill(pDefenderNPC);
                        }
        ...
        if(pMap)
        {
                         ...
                         if(m_uiSpeech_Timerz < diff)
            {
                ...
                break;
                    case 1:
                TeleportPlayers();
                if(Creature* pDefenderNPC = GetClosestCreatureWithEntry(me, NPC_GORKUN_IRONSKULL_END, 3025.0f))
                {
                        pDefenderNPC->CastSpell(pDefenderNPC, SPELL_FROST_BOMB, false);
