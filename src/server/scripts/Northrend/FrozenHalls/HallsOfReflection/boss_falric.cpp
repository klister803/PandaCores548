    ...
    struct boss_falricAI : public ScriptedAI
    {
       ...
       uint32 pSummon;
        ...
        void Reset()
        {
          ...
          me->SetVisible(false);
          if (m_pInstance && m_pInstance->GetData(DATA_FALRIC_EVENT) == IN_PROGRESS)
              m_pInstance->SetData(DATA_FALRIC_EVENT, NOT_STARTED);
        }
        ...
        void EnterCombat(Unit* pVictim)
        {
            ...
            m_pInstance->DoUpdateWorldState(WORLD_STATE_HOR_WAVE_COUNT, 5);
        }
