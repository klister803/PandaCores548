...
{
    ACTION_NULL                                 = 0,
    ACTION_TOWER_OF_STORMS                      = 1,
    ACTION_TOWER_OF_FLAMES                      = 2,
    ACTION_TOWER_OF_FROST                       = 3,
    ACTION_TOWER_OF_LIFE                        = 4,
    ACTION_VEHICLE_RESPAWN                      = 5,
    ACTION_COLOSSUS_COUNT                       = 6
};
    ...
    struct boss_flame_leviathanAI : public BossAI
    {
        ...
        void ActiveTowers()
        {
            if (instance)
            {
                ...
                {
                    ++towerCount;
                    me->CastSpell(me, SPELL_BUFF_TOWER_OF_STORMS, false);
                ...
                {
                    ++towerCount;
                    me->CastSpell(me, SPELL_BUFF_TOWER_OF_FLAMES, false);
                ...
                {
                    ++towerCount;
                    me->CastSpell(me, SPELL_BUFF_TOWER_OF_FROST, false);
        ...
        void InstallAdds(bool bReset = false)
        {
            ...
            else
            {
                ...
                for(std::list<Creature*>::const_iterator itr = lSeats.begin(); itr != lSeats.end(); itr++)
                {
                    if(Vehicle* pSeat = (*itr)->GetVehicleKit())
                    {
	...
	bool OnTrigger(Player* pPlayer, const AreaTriggerEntry* /*pAt*/, bool /*enter*/)
    {
        ...
        if(Creature* vehicle = pPlayer->GetVehicleCreatureBase())
        {
            if (!vehicle->HasAura(SPELL_AUTO_REPAIR) && !pPlayer->isInCombat())
            {
                ...
                if (instance)
                    instance->SetData(DATA_UNBROKEN, 1);
            }
        }
        return true;
    }
...
class achievement_three_car_garage_chopper : public AchievementCriteriaScript
{
    public:
        ...
        bool OnCheck(Player* player, Unit* /*target*/)
        {
            ...
            if (player->GetVehicleBase()->GetEntry() == VEHICLE_CHOPPER)
                return true;
...
class achievement_three_car_garage_siege : public AchievementCriteriaScript
{
    public:
        ...
        bool OnCheck(Player* player, Unit* /*target*/)
        {
            ...
            if (player->GetVehicleBase()->GetEntry() == VEHICLE_SIEGE)
                return true;
...
class achievement_three_car_garage_demolisher : public AchievementCriteriaScript
{
    public:
        ...
        bool OnCheck(Player* player, Unit* /*target*/)
        {
            ...
            if (player->GetVehicleBase()->GetEntry() == VEHICLE_DEMOLISHER)
                return true;
...
class achievement_shutdown : public AchievementCriteriaScript
{
    public:
        achievement_shutdown() : AchievementCriteriaScript("achievement_shutdown")
        {
        }
        ...
        bool OnCheck(Player* player, Unit* target)
        {
            ...
            if (Creature * fl = target->ToCreature())
                if (boss_flame_leviathan::boss_flame_leviathanAI* flAI = CAST_AI(boss_flame_leviathan::boss_flame_leviathanAI, fl->AI()))
                    if (!flAI->isShutdown())
                        return true;
...
class achievement_unbroken : public AchievementCriteriaScript
{
    public:
        ...
        bool OnCheck(Player* player, Unit* target)
        {
            ...
            if (instance && instance->GetData(DATA_UNBROKEN))
                return true;
